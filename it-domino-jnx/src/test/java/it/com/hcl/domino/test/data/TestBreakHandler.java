...
public class TestBreakHandler extends AbstractNotesRuntimeTest {
	...
	public void testBreakHandler() throws Exception {
		...
		withTempDb((database) -> {
			...
			Set<Integer> someIds = database
					.getAllNoteIds(EnumSet.of(DocumentClass.DATA), false)
					.stream()
					.limit(5000)
					.collect(Collectors.toSet());

			AtomicInteger breakHandlerInvocations = new AtomicInteger();
			...
			try {
				client.runInterruptable(() -> {
					...
					int oldVal = breakHandlerInvocations.getAndIncrement();
					if (oldVal==1) {
			...
			assertTrue(breakHandlerInvocations.get() > 1);
			assertTrue(receivedBreakSignal);
		});
