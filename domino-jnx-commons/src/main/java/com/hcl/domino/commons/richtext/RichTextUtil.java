...
/**
 * Contains common routines for working with rich text in an implementation-independent way.
 * 
 * @since 1.0.15
 */
public enum RichTextUtil {
	...
	/**
	 * Creates a {@link AbstractCDRecord} implementation wrapper for the provided record data.
	 * 
	 ...
	 */
	public static AbstractCDRecord<?> encapsulateRecord(short signature, ByteBuffer data) {
	...
	/**
	 * Re-encapsulates the provided record (such as one provided by {@link #encapsulateRecord(short, ByteBuffer)}) with
	 * the new encapsulation interface.
	 * 
	 * @param record the {@link AbstractCDRecord} implementation to re-encapsulate
	 * @param encapsulationClass the new interface to use
	 * @return a {@link RichTextRecord} implementation using the data from {@code record} and implementing the new interface
	 */
	public static RichTextRecord<?> reencapsulateRecord(AbstractCDRecord<?> record, Class<? extends RichTextRecord<?>> encapsulationClass) {
		if(record instanceof GenericBSIGRecord) {
			...
			throw new IllegalArgumentException(MessageFormat.format("Unable to determine encapsulation for {0}", record.getClass().getName()));
		}
	}
	
	public static void writeImageRecords(RichTextWriter w, InputStream is, long fileSize, int imgWidth, int imgHeight, int resizeToWidth, int resizeToHeight, CDImageHeader.ImageType imageType) {
//		write graphic header
		...
		int fullSegments = (int)(fileSize / RichTextConstants.IMAGE_SEGMENT_MAX);
		int segments = fullSegments;
		int dataBytesInLastSegment = (int)(fileSize - fullSegments * RichTextConstants.IMAGE_SEGMENT_MAX);
		if (dataBytesInLastSegment>0) {
			segments++;
		}
		int fSegments = segments;
		...
		byte[] buf = new byte[RichTextConstants.IMAGE_SEGMENT_MAX];
		int len;
		int bytesRead = 0;
			...
			for (int i=0; i<segments; i++) {
				...
				if (i<(segments-1)) {
					if (len<RichTextConstants.IMAGE_SEGMENT_MAX) {
						throw new IllegalStateException(format("The InputStream returned {0} instead of {1}", (bytesRead+len), fileSize));
					}
				}
				else {
					//last segment
					if (len < 0) {
						throw new IllegalStateException(format("The InputStream returned {0} instead of {1}", bytesRead, fileSize));
					}
				}
				bytesRead += len;
				int fLen = len;
	...
	public static void writeScriptLibrary(RichTextWriter w, String script, CDEvent.ActionType libraryType) {
		byte[] bytes = script.getBytes(LMBCS);
		...
		for(int i = 0; i < segCount; i++) {
			int dataOffset = RichTextConstants.BLOBPART_SIZE_CAP * i;
			short dataSize = (short)Math.min(paddedLength - dataOffset, RichTextConstants.BLOBPART_SIZE_CAP);
			short segSize = (short)(dataSize + (dataSize % 2));
			byte[] segData = Arrays.copyOfRange(bytes, dataOffset, dataOffset+dataSize);
