...
import com.hcl.domino.commons.org.apache.commons.mail.util.MimeMessageUtils;
...
final class EmailUtils {
  ...
  static {
    ...
    // safe chars
    EmailUtils.SAFE_URL.set('-');
    EmailUtils.SAFE_URL.set('_');
    EmailUtils.SAFE_URL.set('.');
    EmailUtils.SAFE_URL.set('*');
    EmailUtils.SAFE_URL.set('+');
    EmailUtils.SAFE_URL.set('$');
  ...
  static String encodeUrl(final String input) throws UnsupportedEncodingException {
    ...
    for (final byte c : input.getBytes(EmailUtils.US_ASCII)) {
      int b = c;
      if (b < 0) {
        b = 256 + b;
      }
      if (EmailUtils.SAFE_URL.get(b)) {
        builder.append((char) b);
      } else {
        builder.append(EmailUtils.ESCAPE_CHAR);
        final char hex1 = Character.toUpperCase(Character.forDigit(b >> 4 & 0xF, EmailUtils.RADIX));
        final char hex2 = Character.toUpperCase(Character.forDigit(b & 0xF, EmailUtils.RADIX));
        builder.append(hex1);
        builder.append(hex2);
      }
    }
   ...
   * @param message the exception message you would like to see if the object is
   *                {@code null}
   ...
   * {@code 'z'},
   * the ASCII printable characters, will be used, unless letters and numbers are
   * both {@code false},
   * in which case, start and end are set to {@code 0} and
   ...
   * source of randomness. By seeding a
   * single {@link Random} instance with a fixed seed and using it for each call,
  ...
  static String randomAlphabetic(final int count) {
    return EmailUtils.random(count, 0, 0, true, false, null, EmailUtils.RANDOM);
  }
