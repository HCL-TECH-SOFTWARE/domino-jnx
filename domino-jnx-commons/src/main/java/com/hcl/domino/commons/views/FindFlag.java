...
/**
 * These flags are used by {@code JNADominoCollection#findByKey(Set, Object...)} (NIFFindByKey)
...
public enum FindFlag {
	...
	public static short toBitMask(Set<FindFlag> findSet) {
		int result = 0;
		if (findSet!=null) {
			for (FindFlag currFind : values()) {
	...
	public static int toBitMaskInt(Set<FindFlag> findSet) {
		int result = 0;
		if (findSet!=null) {
			for (FindFlag currFind : values()) {
				if (findSet.contains(currFind)) {
