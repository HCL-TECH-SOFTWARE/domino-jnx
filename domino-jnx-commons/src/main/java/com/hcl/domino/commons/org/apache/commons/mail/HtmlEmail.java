 ...
 * <p>
 * To send an email in HTML, one should create a {@code HtmlEmail}, then
 ...
 * </pre>
 * <p>
 * Embedded entities are tracked by their name, which for {@code File}s is
...
public class HtmlEmail extends MultiPartEmail {
  ...
  private static final String HTML_MESSAGE_START = "<html><body><pre>"; //$NON-NLS-1$

  /** suffix for default HTML mail. */
  private static final String HTML_MESSAGE_END = "</pre></body></html>"; //$NON-NLS-1$
  ...
  private void build() throws MessagingException, EmailException {
    ...
    rootContainer.setSubType("mixed"); //$NON-NLS-1$

    // determine how to form multiparts of email

    if (EmailUtils.isNotEmpty(this.html) && this.inlineEmbeds.size() > 0) {
      // If HTML body and embeds are used, create a related container and add it to
      // the root container
      bodyEmbedsContainer = new MimeMultipart("related"); //$NON-NLS-1$
      bodyContainer = bodyEmbedsContainer;
      this.addPart(bodyEmbedsContainer, 0);

      // If TEXT body was specified, create a alternative container and add it to the
      // embeds container
      if (EmailUtils.isNotEmpty(this.text)) {
        bodyContainer = new MimeMultipart("alternative"); //$NON-NLS-1$
        final BodyPart bodyPart = this.createBodyPart();
        try {
      ...
      if (this.inlineEmbeds.size() > 0 || this.isBoolHasAttachments()) {
        ...
        rootContainer.setSubType("alternative"); //$NON-NLS-1$
      }
    }
    ...
    if (EmailUtils.isNotEmpty(this.html)) {
      ...
      if (contentType == null || !contentType.equals(EmailConstants.TEXT_HTML)) {
        ...
        if (EmailUtils.isNotEmpty(this.charset)) {
          msgHtml.setContent(this.html, EmailConstants.TEXT_HTML + "; charset=" + this.charset); //$NON-NLS-1$
        } else {
  ...
  public String embed(final DataSource dataSource, final String name) throws EmailException {
    ...
    if (this.inlineEmbeds.containsKey(name)) {
      final InlineImage ii = this.inlineEmbeds.get(name);
      ...
      throw new EmailException(
          MessageFormat.format(
              "embedded DataSource ''{0}'' is already bound to name {1}; existing names cannot be rebound",
              name, ii.getDataSource().toString()));
    }
  ...
  public String embed(final DataSource dataSource, final String name, final String cid)
    ...
    try {
      ...
      mbp.setContentID("<" + encodedCid + ">"); //$NON-NLS-1$ //$NON-NLS-2$
   ...
   * <p>
   * While functionally the same as passing {@code FileDataSource} to
   ...
   *                        also see {@link jakarta.mail.internet.MimeBodyPart}
   *                        for definitions
   * @since 1.1
   */
  public String embed(final File file, final String cid) throws EmailException {
    ...
    try {
      ...
      throw new EmailException(MessageFormat.format("couldn''t get canonical path for {0}", file.getName()), ioe);
    }

    // check if a FileDataSource for this name has already been attached;
    // if so, return the cached CID value.
    if (this.inlineEmbeds.containsKey(file.getName())) {
      final InlineImage ii = this.inlineEmbeds.get(file.getName());
      ...
      try {
        ...
        throw new EmailException(
            MessageFormat.format(
                "couldn''t get canonical path for file {0}which has already been embedded",
      ...
      throw new EmailException(
          MessageFormat.format(
              "embedded name ''{0}'' is already bound to file {1}; existing names cannot be rebound",
              file.getName(), existingFilePath));
    }

    // verify that the file is valid
    if (!file.exists()) {
      throw new EmailException(MessageFormat.format("file {0} doesn''t exist", filePath));
    }
    if (!file.isFile()) {
      throw new EmailException(MessageFormat.format("file {0} isn''t a normal file", filePath));
    }
    if (!file.canRead()) {
      throw new EmailException(MessageFormat.format("file {0} isn''t readable", filePath));
    }
   ...
   * @throws EmailException when URL supplied is invalid or if {@code name} is
   *                        null
   *                        or empty; also see
   *                        {@link jakarta.mail.internet.MimeBodyPart} for
   *                        definitions
   ...
   * <p>
   * While functionally the same as passing {@code URLDataSource} to
   * {@link #embed(DataSource, String, String)}, this method attempts
  ...
  public String embed(final URL url, final String name) throws EmailException {
    ...
    if (this.inlineEmbeds.containsKey(name)) {
      ...
      throw new EmailException(
          MessageFormat.format(
              "embedded name ''{0}'' is already bound to URL {1}; existing names cannot be rebound",
              name, urlDataSource.getURL()));
    }
  ...
  public Email setMsg(final String msg) throws EmailException {
    ...
    final StringBuffer htmlMsgBuf = new StringBuffer(
        msg.length()
            + HtmlEmail.HTML_MESSAGE_START.length()
            + HtmlEmail.HTML_MESSAGE_END.length());

    htmlMsgBuf.append(HtmlEmail.HTML_MESSAGE_START)
        .append(msg)
        .append(HtmlEmail.HTML_MESSAGE_END);
