...
public class NotesDateTimeUtils {
	 ...
	 */
	public static Calendar timeDateToCalendar(DominoDateTime timeDate) {
	...
	public static int compareInnards(int[] innards1, int[] innards2) {
		if (!hasDate(innards1)) {
			throw new IllegalArgumentException(format("Innard array #1 does not have a date part: {0}", Arrays.toString(innards1)));
		}
		if (!hasDate(innards2)) {
			throw new IllegalArgumentException(format("Innard array #1 does not have a date part: {0}", Arrays.toString(innards2)));
		}
		if (!hasTime(innards1)) {
			throw new IllegalArgumentException(format("Innard array #1 does not have a time part: {0}", Arrays.toString(innards1)));
		}
		if (!hasTime(innards2)) {
			throw new IllegalArgumentException(format("Innard array #1 does not have a time part: {0}", Arrays.toString(innards2)));
		}
	...
	public static boolean hasTime(int[] innards) {
		if (innards.length!=2) {
			throw new IllegalArgumentException(format("Invalid innard size: {0}, expected 2", innards.length));
		}
	...
	@Deprecated
	public static Calendar createCalendar(int year, int month, int day, int hour, int minute, int second, int millis, TimeZone zone) {
	...
	public static int[] calendarToInnards(Calendar cal) {
		...
		if(hasDate && hasTime) {
			...
			LocalTime localTime = LocalTime.of(cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE), cal.get(Calendar.SECOND), cal.get(Calendar.MILLISECOND)*1000*1000);
