...
public class ViewFormatDecoder {
  ...
  public static DominoViewFormat decodeViewFormat(Document parentDoc, ByteBuffer data) {
    ...
    // Always present
    {
      ...
      for (int i = 0; i < colCount; i++) {
        ViewColumnFormat tempCol =
            readMemory(data, ODSTypes._VIEW_COLUMN_FORMAT, ViewColumnFormat.class);

        // Find the actual size with variable data and re-read
        int varSize = tempCol.getItemNameLength() + tempCol.getTitleLength()
            + tempCol.getFormulaLength() + tempCol.getConstantValueLength();
        ViewColumnFormat fullCol =
            MemoryStructureUtil.newStructure(ViewColumnFormat.class, varSize);
     ...
     */
    ViewTableFormat2 format2 =
        readMemory(data, ODSTypes._VIEW_TABLE_FORMAT2, ViewTableFormat2.class);
    result.read(format2);
    ...
    for (int i = 0; i < format1.getColumnCount(); i++) {
      ViewColumnFormat2 col =
          readMemory(data, ODSTypes._VIEW_COLUMN_FORMAT2, ViewColumnFormat2.class);
      if (col.getSignature() != ViewFormatConstants.VIEW_COLUMN_FORMAT_SIGNATURE2) {
        throw new IllegalStateException(
            MessageFormat.format("Read unexpected VIEW_COLUMN_FORMAT2 signature: 0x{0}",
                Integer.toHexString(col.getSignature())));
      }
    ...
    ViewTableFormat3 format3 =
        readMemory(data, ODSTypes._VIEW_TABLE_FORMAT3, ViewTableFormat3.class);
    result.read(format3);
    ...
    for (int i = 0; i < format1.getColumnCount(); i++) {
      ...
      if (hideWhenLen > 0 || twistieLen > 0) {
        // add hide when formula and twistie resource to the vardata of VIEW_COLUMN_FORMAT2 although
        ...
        if (twistieLen > 0) {
          ...
          CDResource res = MemoryStructureUtil.newStructure(CDResource.class,
              twistieLen - MemoryStructureUtil.sizeOf(CDResource.class));
          res.getData().put(buf);
    ...
    for (int i = 0; i < format1.getColumnCount(); i++) {
      ...
      if (fmt.getFlags().contains(ViewColumnFormat2.Flag3.ExtDate)) {
        ...
        ViewColumnFormat3 tempCol =
            MemoryStructureUtil.forStructure(ViewColumnFormat3.class, () -> tempBuf);
        if (tempCol.getSignature() != ViewFormatConstants.VIEW_COLUMN_FORMAT_SIGNATURE3) {
          throw new IllegalStateException(
              "Encountered unexpected signature when looking for VIEW_COLUMN_FORMAT3: 0x"
                  + Integer.toHexString(tempCol.getSignature()));
        }

        // Re-read with variable data
        int varLen = tempCol.getDateSeparator1Length() + tempCol.getDateSeparator2Length()
            + tempCol.getDateSeparator3Length()
    ...
    for (int i = 0; i < format1.getColumnCount(); i++) {
      ...
      if (fmt.getFlags().contains(ViewColumnFormat2.Flag3.NumberFormat)) {
        ...
        ByteBuffer tempBuf = subBuffer(data, len);
        ViewColumnFormat4 tempCol =
            MemoryStructureUtil.forStructure(ViewColumnFormat4.class, () -> tempBuf);
        if (tempCol.getSignature() != ViewFormatConstants.VIEW_COLUMN_FORMAT_SIGNATURE4) {
          throw new IllegalStateException(
              "Encountered unexpected signature when looking for VIEW_COLUMN_FORMAT4: 0x"
                  + Integer.toHexString(tempCol.getSignature()));
        }

        // Re-read with variable data
        long varLen = tempCol.getCurrencySymbolLength() + tempCol.getDecimalSymbolLength()
            + tempCol.getMilliSeparatorLength()
            + tempCol.getNegativeSymbolLength();
        ByteBuffer buf = readBuffer(data, len + varLen);
        ViewColumnFormat4 col4 =
            MemoryStructureUtil.newStructure(ViewColumnFormat4.class, (int) varLen);
    ...
    for (int i = 0; i < format1.getColumnCount(); i++) {
      ...
      if (fmt.getFlags().contains(ViewColumnFormat2.Flag3.NamesFormat)) {
        ...
        ByteBuffer tempBuf = subBuffer(data, len);
        ViewColumnFormat5 tempCol =
            MemoryStructureUtil.forStructure(ViewColumnFormat5.class, () -> tempBuf);
        if (tempCol.getSignature() != ViewFormatConstants.VIEW_COLUMN_FORMAT_SIGNATURE5) {
          throw new IllegalStateException(
              "Encountered unexpected signature when looking for VIEW_COLUMN_FORMAT5: 0x"
                  + Integer.toHexString(tempCol.getSignature()));
        }
        ...
        ViewColumnFormat5 col5 = MemoryStructureUtil.newStructure(ViewColumnFormat5.class,
            totalLen - MemoryStructureUtil.sizeOf(ViewColumnFormat5.class));
    ...
    // TODO figure out where these ghost bits come from. I'd thought they were "ghost" shared-column
    // names, but
    // it turns out they were instead hidden column titles. It seems in practice (test data and
    // mail12.ntf) that
    // they're all 0, so the below algorithm "works" to move past them
    while (data.remaining() > 1
        && data.getShort(data.position()) != ViewFormatConstants.VIEW_COLUMN_FORMAT_SIGNATURE6) {
    ...
    for (int i = 0; i < format1.getColumnCount(); i++) {
      ...
      if (fmt.getFlags().contains(ViewColumnFormat2.Flag3.ExtendedViewColFmt6)) {
        ...
        ByteBuffer tempBuf = subBuffer(data, len);
        ViewColumnFormat6 tempCol =
            MemoryStructureUtil.forStructure(ViewColumnFormat6.class, () -> tempBuf);
        if (tempCol.getSignature() != ViewFormatConstants.VIEW_COLUMN_FORMAT_SIGNATURE6) {
          throw new IllegalStateException(
              "Encountered unexpected signature when looking for VIEW_COLUMN_FORMAT6: 0x"
                  + Integer.toHexString(tempCol.getSignature()));
        }
        ...
        int totalLen = tempCol.getLength();
        ByteBuffer buf = readBuffer(data, totalLen);
        ViewColumnFormat6 col6 = MemoryStructureUtil.newStructure(ViewColumnFormat6.class,
            totalLen - MemoryStructureUtil.sizeOf(ViewColumnFormat6.class));
  ...
  public static DominoCalendarFormat decodeCalendarFormat(ByteBuffer data) {
    ...
    ViewCalendarFormat format1 =
        readMemory(data, ODSTypes._VIEW_CALENDAR_FORMAT, ViewCalendarFormat.class);
    result.read(format1);
