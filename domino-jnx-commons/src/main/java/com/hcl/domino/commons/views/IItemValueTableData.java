...
public interface IItemValueTableData {
  ...
  /**
   * Returns the data type of an item value by its index, e.g.
   ...
   * <li>{@link ItemDataType#TYPE_NUMBER_RANGE} - {@link List} with {@link Double}
   * values for number lists or double[] values for number ranges (not sure if
   * Notes views really supports them)</li>
   * <li>{@link ItemDataType#TYPE_TIME} - {@link Calendar}; if
   * {@link #setPreferNotesTimeDates(boolean)} is called, we return
   * {@link DominoDateTime} instead</li>
   * <li>{@link ItemDataType#TYPE_TIME_RANGE} - {@link List} with {@link Calendar}
   * values for datetime lists or Calendar[] values for datetime ranges; if
   * {@link #setPreferNotesTimeDates(boolean)} is called, we return
   * {@link DominoDateTime} and {@link DominoDateRange} instead</li>
   * </ul>
   * 
   * @param index item index between 0 and {@link #getItemsCount()}
  ...
  /**
   * Returns whether methods like {@link #getItemValue(int)} should return
   * {@link DominoDateTime}
   * instead of {@link Calendar} and {@link DominoDateRange} instead of
   * Calendar[].
   * 
  ...
  /**
   * Sets whether methods like {@link #getItemValue(int)} should return
   * {@link DominoDateTime}
   * instead of {@link Calendar} and {@link DominoDateRange} instead of
   * Calendar[].
   * 
