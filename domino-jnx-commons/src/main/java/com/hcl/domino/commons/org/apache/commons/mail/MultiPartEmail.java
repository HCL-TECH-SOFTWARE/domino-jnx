...
public class MultiPartEmail extends Email {
  ...
  public MultiPartEmail attach(
    ...
    return this.attach(ds, name, description, EmailAttachment.ATTACHMENT);
  }
  ...
  public MultiPartEmail attach(final EmailAttachment attachment)
    ...
    if (url == null) {
      ...
      try {
        ...
        if (!file.exists()) {
          throw new IOException(MessageFormat.format("\"{0}\" does not exist", fileName));
        }
        result = this.attach(
        ...
        throw new EmailException(MessageFormat.format("Cannot attach file \"{0}\"", fileName), e);
      }
    } else {
      result = this.attach(
          url,
  ...
  public MultiPartEmail attach(final File file)
    ...
    try {
      ...
      return this.attach(fds, file.getName(), null, EmailAttachment.ATTACHMENT);
    } catch (final IOException e) {
      throw new EmailException(MessageFormat.format("Cannot attach file \"{0}\"", fileName), e);
    }
  }
  ...
  public MultiPartEmail attach(final URL url, final String name, final String description)
      throws EmailException {
    return this.attach(url, name, description, EmailAttachment.ATTACHMENT);
  }
  ...
  public MultiPartEmail attach(
    ...
    try {
      ...
      throw new EmailException(MessageFormat.format("Invalid URL set:{0}", url), e);
    }

    return this.attach(new URLDataSource(url), name, description, disposition);
  }
   ...
   * @throws MessagingException An error occurred while getting the primary body
   *                            part.
   * @since 1.0
   */
  ...
  public Email setMsg(final String msg) throws EmailException {
    ...
    try {
      final BodyPart primary = this.getPrimaryBodyPart();
