...
import com.hcl.domino.commons.org.apache.commons.mail.util.IDNEmailAddressConverter;
...
public abstract class Email {
  ...
  /**
   * @deprecated since 1.3, use {@link EmailConstants#MAIL_TRANSPORT_PROTOCOL}
   *             instead
   */
  @Deprecated
   ...
   *             {@link EmailConstants#MAIL_SMTP_SOCKET_FACTORY_FALLBACK} instead
   */
  @Deprecated
  ...
  /**
   * @deprecated since 1.3, use
   *             {@link EmailConstants#MAIL_SMTP_SOCKET_FACTORY_CLASS} instead
   */
  @Deprecated
  ...
  /**
   * @deprecated since 1.3, use
   *             {@link EmailConstants#MAIL_SMTP_SOCKET_FACTORY_PORT} instead
   */
  @Deprecated
  public static final String MAIL_SMTP_SOCKET_FACTORY_PORT = EmailConstants.MAIL_SMTP_SOCKET_FACTORY_PORT;

  /**
   * @deprecated since 1.3, use {@link EmailConstants#MAIL_SMTP_CONNECTIONTIMEOUT}
   *             instead
   */
  @Deprecated
  ...
  /**
   * If set to true, and a message has some valid and some invalid addresses, send
   * the message anyway,
   * reporting the partial failure with a SendFailedException.
   * If set to false (the default), the message is not sent to any of the
   * recipients
  ...
  public void buildMimeMessage() throws EmailException {
    ...
    try {
      ...
      if (this.fromAddress != null) {
        ...
        if (this.session.getProperty(EmailConstants.MAIL_SMTP_FROM) == null
            && this.session.getProperty(EmailConstants.MAIL_FROM) == null) {
      ...
      if (this.headers.size() > 0) {
        for (final Map.Entry<String, String> entry : this.headers.entrySet()) {
          final String foldedValue = this.createFoldedHeaderValue(entry.getKey(), entry.getValue());
   ...
   * @throws EmailException Thrown when the supplied address, name or charset were
   *                        invalid.
   */
  ...
  /**
   * Set the charset of the message. Please note that you should set the charset
   * before
   ...
   *                                                      invalid
   * @throws java.nio.charset.UnsupportedCharsetException if no support for the
   *                                                      named charset
   ...
   *                 if searched in java:comp/env if name does not start with
   *                 "java:"
   * @throws IllegalArgumentException if the JNDI name is null or empty
   * @throws NamingException          if the resource cannot be retrieved from
   *                                  JNDI directory
   * @since 1.1
   */
   ...
   * {@link #send()}
   * may throw a {@link jakarta.mail.SendFailedException}, even if partial send
   * mode is enabled (emails
   * to valid addresses will be transmitted). In case the email server does not
   * reject
   * invalid addresses immediately, but return a bounce message, no exception will
   * be thrown
