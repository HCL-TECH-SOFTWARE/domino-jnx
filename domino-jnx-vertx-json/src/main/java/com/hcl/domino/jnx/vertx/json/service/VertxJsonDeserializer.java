...
public class VertxJsonDeserializer extends AbstractJsonDeserializer {
	...
	public Document fromJson(String json) {
		...
		Document doc = this.targetDocument == null ? targetDatabase.createDocument() : this.targetDocument;
		...
		jsonObj.stream()
			...
			.forEach(entry -> {
				...
				if(value == null) {
					...
					doc.replaceItemValue(itemName, JsonUtil.convertStringValue(doc.getParentDatabase().getParentDominoClient(), this.detectDateTime, this.dateTimeItems, itemName, value.toString()));
					...
					while(iter.hasNext()) {
						Object arrVal = iter.next();
						if(arrVal == null) {
							result.add(null);
						} else if(!areCompatibleTypes(arrVal, arrayType)) {
							...
							if(arrVal instanceof Number) {
								...
								result.add(JsonUtil.convertStringValue(doc.getParentDatabase().getParentDominoClient(), this.detectDateTime, this.dateTimeItems, itemName, arrVal.toString()));
	...
	private boolean areCompatibleTypes(Object a, Object b) {
		if(a == null || b == null ) {
			return true;
		} else {
