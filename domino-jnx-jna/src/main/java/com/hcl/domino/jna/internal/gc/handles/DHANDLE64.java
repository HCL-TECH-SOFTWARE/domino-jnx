 ...
 */
public abstract class DHANDLE64 implements DHANDLE {
  private boolean disposed;

  public static DHANDLE64 newInstance(long hdl) {
    return AccessController.doPrivileged((PrivilegedAction<DHANDLE64>) () -> new ByValue(hdl));
  }

  @Override
  public <R> R _lockHandleAccess(HandleAccess<DHANDLE.ByValue, R> handleAccess) {
    return AccessController.doPrivileged((PrivilegedAction<R>) () -> {
      Lock lock = JNAGCUtil.getHandleLock(DHANDLE64.this);
      lock.lock();
      try {
        checkDisposed();

        DHANDLE64.ByValue newHdl = new DHANDLE64.ByValue(this.getValue());
  ...
  public static DHANDLE64 newInstance(final Pointer peer) {
    return AccessController.doPrivileged((PrivilegedAction<DHANDLE64>) () -> new ByReference(peer));
  }

  public static class ByReference extends DHANDLE64 implements DHANDLE.ByReference {
    Pointer value;
  ...
  public static class ByValue extends DHANDLE64 implements DHANDLE.ByValue {
    long value;
  ...
  public String toString() {
    return MessageFormat.format("DHANDLE64 [handle={0}]", getValue()); //$NON-NLS-1$
  }
