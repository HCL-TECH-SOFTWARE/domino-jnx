 ...
 */
public class JNAServerAdmin extends BaseJNAAPIObject<JNAServerAdminAllocations>
  ...
  public void markInService(String server, String filePath) {
    try (DisposableMemory dbPathMem =
        JNADominoUtils.constructNetPath(getParentDominoClient(), server, filePath)) {
  ...
  public DominoSSOToken generateSSOToken(String orgName, String configName, String userName,
    ...
    NotesTimeDateStruct creationDateStruct =
        creationDate == null ? null : NotesTimeDateStruct.newInstance(creationDate);
    NotesTimeDateStruct expirationDateStruct =
        expirationDate == null ? null : NotesTimeDateStruct.newInstance(expirationDate);
    ...
    short result =
        NotesCAPI.get().SECTokenGenerate(null, orgNameMem, configNameMem, userNameCanonicalMem,
            creationDateStruct, expirationDateStruct,
            retmhToken, enableRenewal ? NotesConstants.fSECToken_EnableRenewal : 0,
            renewalDate == null ? null : renewalDate.getPointer());
    NotesErrorUtils.checkResult(result);
    ...
    try (LockedMemory mem = Mem.OSMemoryLock(hToken);) {
      ...
      if (tokenData.getNumDomains() > 0 && tokenData.getDomainListHandle() != 0) {
        try (LockedMemory domainsMem = Mem.OSMemoryLock(tokenData.getDomainListHandle())) {
          ...
          domains =
              NotesStringUtils.fromLMBCSStringList(ptrDomains, tokenData.getNumDomains() & 0xffff);
        }
      } else {
      ...
      DefaultDominoSSOToken ssoToken = new DefaultDominoSSOToken(name, domains, isSecureOnly, data,
          renewalDate == null ? null : new JNADominoDateTime(renewalDate));
      return ssoToken;
  ...
  public List<String> getServerClusterMates(String serverName, ClusterLookup lookupMode) {
    Memory serverNameCanonical = serverName == null ? null
        : NotesStringUtils.toLMBCS(NotesNamingUtils.toCanonicalName(serverName), true);

    DHANDLE.ByReference phList = DHANDLE.newInstanceByReference();

    short result = NotesCAPI.get().NSGetServerClusterMates(serverNameCanonical,
        lookupMode == null ? 0 : lookupMode.getValue(), phList);
  ...
  public MultiDatabaseAccessInfo computeDatabaseAccess(String server,
    ...
    JNAUserNamesList usernamesList =
        NotesNamingUtils.buildNamesList(getParentDominoClient(), server, username);
  ...
  public MultiDatabaseAccessInfo computeDatabaseAccess(String server,
    ...
    if (forceNewScan || !allocations.isACLCacheInitialized(server)) {
      ...
      {
        JNAUserNamesList namesListFullAccess =
            NotesNamingUtils.buildNamesList((JNADominoClient) client, server, idUsername);
        NotesNamingUtils.setPrivileges(namesListFullAccess,
            EnumSet.of(Privileges.Authenticated, Privileges.FullAdminAccess));

        String primaryDirectory =
            getParentDominoClient().openUserDirectory(null).getPrimaryDirectoryPath()
                .orElseThrow(
                    () -> new IllegalStateException("Unable to identify primary directory path"));
      ...
      client
          ...
          .forEach((fileEntry) -> {
            if (fileEntry instanceof DatabaseData) {
              ...
              if (fHasFullAccess) {
                try {
                  namesListFullAccess = NotesNamingUtils.buildNamesList((JNADominoClient) client,
                      dbData.getServer(), idUsername);
                  NotesNamingUtils.setPrivileges(namesListFullAccess,
                      EnumSet.of(Privileges.Authenticated, Privileges.FullAdminAccess));

                  db = new JNADatabase(getParentDominoClient(), dbData.getServer(),
                      dbData.getFilePath(),
                      EnumSet.noneOf(OpenDatabase.class), namesListFullAccess);
                } catch (DominoException e) {
                  db = null;
              ...
              if (db == null && (dbOpenException == null
                  || (dbOpenException != null && dbOpenException.getId() == 582))) { // not
                                                                                     // authorized
                try {
                  db = new JNADatabase(getParentDominoClient(), dbData.getServer(),
                      dbData.getFilePath(),
                  ...
                  // System.out.println("ERR "+e.getId()+" opening database
                  // "+dbData.getServer()+"!!"+dbData.getFilePath());
                  // e.printStackTrace();
                }
              }
              ...
              if (db != null) {
                try {
                  ...
                  System.err.println(MessageFormat.format("ERR {0} opening database {1}!!{2}",
                      e.getId(), dbData.getServer(), dbData.getFilePath()));
                  e.printStackTrace();
                } finally {
                  db.close();
                }
  ...
  public void registerCrossCertificate(String certFilePath, String certPW, String certLogPath,
    ...
    short result = NotesCAPI.get().SECKFMGetCertifierCtx(certFilePathMem, kfmPwd, certLogPathMem,
        tdStructByRef, retCertNameMem,
    ...
    try {
      ...
      NotesCallbacks.REGSIGNALPROC statusCallback = ptrMessage -> {
        if (msgHandler != null) {
      ...
      result = LockUtil.lockHandle(rethKfmCertCtx, (rethKfmCertCtxByVal) -> {
        return NotesCAPI.get().REGCrossCertifyID(rethKfmCertCtxByVal, (short) 0,
            regServerCanonicalMem,
            idFilePathMem,
            locationMem, commentMem, forwardAddressMem, (short) 0, statusCallback,
            errorPathNameMem);
  ...
  public void setDefaultServerStatusLine(ServerStatusLine statusLine) {
    ...
    if (!(statusLine instanceof JNAServerStatusLine)) {
      throw new IllegalArgumentException(MessageFormat.format(
          "Unable to handle ServerStatusLine of type {0}", statusLine.getClass().getName()));
    }
  ...
  public void openServerConsole(String serverName, ConsoleHandler handler) {
    String serverNameCanonical =
        StringUtil.isEmpty(serverName) ? getParentDominoClient().getIDUserName()
            : NotesNamingUtils.toCanonicalName(serverName);
...

    {
      ...
      // otherwise that error status is just returned asynchronously via ASYNC_CONTEXT and the
      // required
    ...
    try {
      NotesCallbacks.ASYNCNOTIFYPROC callback = (p1, p2) -> {
      };
