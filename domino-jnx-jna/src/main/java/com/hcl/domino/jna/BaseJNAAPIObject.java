...
/**
 * Base class for all objects in the JNA API that have resources on the C API side
 * 
 * @author Karsten Lehmann
 *
 * @param <AT> subclass of {@link APIObjectAllocations} that contain the C API handles and could to dispose them
 */
...
public abstract class BaseJNAAPIObject<AT extends APIObjectAllocations> extends BaseAPIObject<AT> {
	 ...
	 * returns the default Java return value or something meaningful (e.g. returning the
	 * handle value and data read from the handle).
	 */
...
//	}
	...
	/**
	 * Overriding {@link #toString()} with a final method to ensure
	 * API objects are completely initialized until this method is called.<br>
	 * <br>
	 * It is expected that subclasses call {@link #setInitialized()} in their
	 * constructor as last method to ensure all is set up properly. Without calling this
	 * method, we just return the default Java {@link #toString()} result here.<br>
	 * <br>
	...
	/**
     * Returns a string representation of the object. Method is called
     * by {@link BaseJNAAPIObject#toString()} if {@link #setInitialized()}
     * has already been called, so it's safe to assume that all
     * required handles have been read/stored.<br>
     * If {@link #setInitialized()} has not been called, {@link BaseJNAAPIObject#toString()}
     * falls back to the default method {@link Object#toString()}.
     *
