...
public class NotesCAPI12 {
	private static volatile INotesCAPI12 m_instance;
	private static volatile INotesCAPI12 m_instanceWithStackLogging;
	
	private static DominoException m_initError;

	public static synchronized INotesCAPI12 get() {
		...
		if (useCallstackLogging) {
			if (m_instanceWithStackLogging==null) {
				m_instanceWithStackLogging = NotesCAPI.wrapWithCrashStackLogging(INotesCAPI12.class, m_instance);
			}
	...
	private static INotesCAPI12 createAPI() {
		...
		if (PlatformUtils.isWindows()) {
			api = Native.load("nnotes", INotesCAPI12.class, libraryOptions); //$NON-NLS-1$
		}
		else {
			api = Native.load("notes", INotesCAPI12.class, libraryOptions); //$NON-NLS-1$
		}
		return api;
	}
