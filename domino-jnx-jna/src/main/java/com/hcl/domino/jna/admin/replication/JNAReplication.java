...
public class JNAReplication extends BaseJNAAPIObject<JNAReplicationAllocations>
  ...
  public void clearReplicationHistory(Database db) {
    ...
    JNADatabaseAllocations dbAllocations =
        (JNADatabaseAllocations) jnaDb.getAdapter(APIObjectAllocations.class);
  ...
  public void setReplicaInfo(Database db, ReplicaInfo replicaInfo) {
    ...
    short result = LockUtil.lockHandle(dbAllocations.getDBHandle(), (hDbByVal) -> {
      return NotesCAPI.get().NSFDbReplicaInfoSet(hDbByVal,
          jnaReplicaInfo.getAdapter(NotesDbReplicaInfoStruct.class));
    });
    NotesErrorUtils.checkResult(result);
  ...
  @Override
  public void saveDocument(Document doc, boolean force, boolean noRevisionHistory,
      boolean keepModTime) {
    ...
    JNADocumentAllocations docAllocations =
        (JNADocumentAllocations) jnaDoc.getAdapter(APIObjectAllocations.class);
  ...
  @Override
  public void getDocuments(Database db, int[] noteIds, Set<OpenDocumentMode>[] docOpenFlags,
      int[] sinceSeqNum,
      Set<GetDocumentsMode> controlFlags, Database objectDb,
      IGetDocumentsCallback getDocumentsCallback,
    ...
    if (objectDb != null) {
      ...
      jnaObjectDbAllocations =
          (JNADatabaseAllocations) jnaObjectDb.getAdapter(APIObjectAllocations.class);
    }
    ...
    if (noteIds.length != docOpenFlags.length) {
      throw new DominoException(0,
          MessageFormat.format(
              "Size of document open flags array does not match note ids array ({0}!={1})",
              docOpenFlags.length, noteIds.length));
    }
    if (noteIds.length != sinceSeqNum.length) {
      throw new DominoException(0,
          MessageFormat.format(
              "Size of sinceSeqNum array does not match note ids array ({0}!={1})",
              sinceSeqNum.length,
              noteIds.length));
    }

    final NotesTimeDateStruct folderSinceTimeStruct = folderSinceTime == null ? null
        : NotesTimeDateStruct.newInstance(JNADominoDateTime.from(folderSinceTime).getInnards());
    ...
    if (docOpenCallback != null) {
      cNoteOpenCallback = (param, hNote, noteId, status) -> {
        ...
        if (hNote == 0) {
          ...
          DHANDLE hNoteObj = DHANDLE.newInstanceByValue(hNote);
    ...
    if (objectWriteCallback != null) {
      cObjectWriteCallback = (param, hNote, oldRRV, status, buffer, bufferSize) -> {
        ...
        try {
          Action action =
              objectWriteCallback.objectChunkWritten(note, oldRRV, statusEx, byteBuf, bufferSize);
          if (action == Action.STOP) {
            stopped.set(Boolean.TRUE);
            return INotesErrorConstants.ERR_CANCEL;
          } else {
    ...
    short result = LockUtil.lockHandles(dbAllocations.getDBHandle(),
        ...
        (hDbByVal, hObjectDbByVal) -> {
          return NotesCAPI.get().NSFDbGetNotes(hDbByVal, noteIds.length, arrNoteIdsMem,
              arrNoteOpenFlagsMem,
              arrSinceSeqNumMem, controlFlagsBitMask, hObjectDbByVal,
              null, cGetNotesCallback, cNoteOpenCallback, cObjectAllocCallback,
              cObjectWriteCallback,
  ...
  @Override
  public List<ReplicationHistorySummary> getReplicationHistory(Database db,
      Set<ReplicationHistoryFlags> flags) {
    checkDisposed();
    ...
    short result = LockUtil.lockHandle(dbAllocations.getDBHandle(), (hDbByVal) -> {
      return NotesCAPI.get().NSFDbGetReplHistorySummary(hDbByVal, dwFlags, rethSummary,
          retNumEntries);
    });
    NotesErrorUtils.checkResult(result);
      ...
      if (!rethSummary.isNull()) {
        ...
        try {
          ...
          for (int i = 0; i < numEntries; i++) {
            NotesReplicationHistorySummaryStruct struct =
                NotesReplicationHistorySummaryStruct.newInstance(currPosPtr);
            struct.read();

            DominoDateTime replicationTime = struct.ReplicationTime == null ? null
                : new JNADominoDateTime(struct.ReplicationTime);
            AclLevel aclLevel =
                DominoEnumUtil.valueOf(AclLevel.class, Short.toUnsignedInt(struct.AccessLevel))
                    .orElseThrow(() -> new IllegalStateException(
                        MessageFormat.format("Cannot identify access level for {0}",
                            Short.toUnsignedInt(struct.AccessLevel))));
            ...
            String serverFilePath =
                NotesStringUtils.fromLMBCS(ptr.share(struct.ServerNameOffset), -1);
            ...
            currPosPtr =
                currPosPtr.share(JNANotesConstants.notesReplicationHistorySummaryStructSize);
          }
        } finally {
