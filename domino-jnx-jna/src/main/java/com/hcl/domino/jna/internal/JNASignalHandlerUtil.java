...
public class JNASignalHandlerUtil {
  private static final ConcurrentHashMap<Thread, IBreakHandler> m_breakHandlerByThread =
      new ConcurrentHashMap<>();
  private static final ConcurrentHashMap<Thread, IProgressListener> m_progressListenerByThread =
      new ConcurrentHashMap<>();
  private static final ConcurrentHashMap<Thread, ReplicationStateListener> m_replicationStateListenerByThread =
      new ConcurrentHashMap<>();
  ...
  public static synchronized void installGlobalBreakHandler() {
    if (!m_breakProcInstalled) {
      try {
        ...
        prevBreakProc =
            AccessController.doPrivileged((PrivilegedExceptionAction<OSSIGBREAKPROC>) () -> {
              return (NotesCallbacks.OSSIGBREAKPROC) NotesCAPI.get()
                  .OSSetBreakSignalHandler(NotesConstants.OS_SIGNAL_CHECK_BREAK, breakProc);
            });
  ...
  public static synchronized void installGlobalProgressHandler() {
    if (!m_progressProcInstalled) {
      try {
        // AccessController call required to prevent SecurityException when running in XPages
        prevProgressProc =
            AccessController.doPrivileged((PrivilegedExceptionAction<OSSIGPROGRESSPROC>) () -> {
              return (NotesCallbacks.OSSIGPROGRESSPROC) NotesCAPI.get()
                  .OSSetProgressSignalHandler(NotesConstants.OS_SIGNAL_PROGRESS, progressProc);
            });
      } catch (PrivilegedActionException e) {
        if (e.getCause() instanceof RuntimeException) {
          throw (RuntimeException) e.getCause();
  ...
  public static synchronized void installGlobalReplicationStateHandler() {
    if (!m_replProcInstalled) {
      try {
        // AccessController call required to prevent SecurityException when running in XPages
        prevReplProc =
            AccessController.doPrivileged((PrivilegedExceptionAction<OSSIGREPLPROC>) () -> {
              return (NotesCallbacks.OSSIGREPLPROC) NotesCAPI.get()
                  .OSSetReplicationSignalHandler(NotesConstants.OS_SIGNAL_REPL, replProc);
            });
      } catch (PrivilegedActionException e) {
        if (e.getCause() instanceof RuntimeException) {
          throw (RuntimeException) e.getCause();
   ...
   * @param <T> result type
   */
  public static <T> T runWithProgress(Callable<T> callable,
   ...
   * 
   * @param callable callable to execute
   * @param replStateListener replication state handler to get notified about replication state
   *        changes
   * @return optional result
