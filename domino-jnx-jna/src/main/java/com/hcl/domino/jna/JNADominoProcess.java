...
public class JNADominoProcess implements DominoProcess {
  ...
  private static Map<Thread, ThreadInfo> threadEnabledForDominoRefCount =
  ...
  private static class DominoPacemakerThread extends Thread {
    ...
    private final boolean nativeProcessInit;
    private final boolean nativeThreadInit;

    public DominoPacemakerThread(int argsCount, StringArray args, boolean nativeProcessInit,
        boolean nativeThreadInit) {
      super("Domino JNX Pacemaker");

      this.argsCount = argsCount;
      this.args = args;
      this.nativeProcessInit = nativeProcessInit;
      this.nativeThreadInit = nativeThreadInit;
    }
    ...
    private DominoInitException toDominoInitException(short result) {
      ...
      // resolve C API init error code without using OSLoadString (using built-in constants) to
      // prevent a crash
      ...
      try {
        ...
        statusMessage = MessageFormat.format("Error initializing Notes runtime, ERR 0x{0}",
            Integer.toHexString(resultAsInt));
      }
      final boolean isRemoteError =
          (result & NotesConstants.STS_REMOTE) == NotesConstants.STS_REMOTE;
    ...
    public void run() {
      ...
      try {
        ...
        capi = NotesCAPI.get(true); // true => do not check if thread is initialized for Domino,
                                    // because we do this
        ...
        this.initException = new DominoInitException(
            "Error loading Notes/Domino shared library. Please make sure that the location of nnotes.dll (Windows), libnotes.so (Linux) or libnotes.dylib is added to the PATH.",
            e);
        notifyCaller();
        return;
      }
      ...
      if (nativeProcessInit && !DominoUtils.isNoInit()) {
        if (this.argsCount == 0) {
        ...
        if (result != 0) {
          ...
          if (debug) {
            System.out.println(
                MessageFormat.format("Domino API could not initialize the process. ERR 0x{0}",
                    Integer.toHexString(resultAsInt)));
          }
      ...
      if (nativeThreadInit) {
        ...
        if (result != 0) {
          ...
          if (debug) {
            System.out.println(
                MessageFormat.format("Domino API could not initialize pacemaker thread. ERR 0x{0}",
                    Integer.toHexString(result)));
          }
          ...
          if (nativeProcessInit && !DominoUtils.isNoTerm()) {
            capi.NotesTerm();
          }
