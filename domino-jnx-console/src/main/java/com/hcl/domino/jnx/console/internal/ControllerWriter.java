...
class ControllerWriter extends Thread {
    ...
    private ObjectStack cmap;
    private boolean stopflag = false;

    public ControllerWriter(DominoConsoleRunner dominoConsole) {
	...
	public void run() {
        ...
        while (!this.stopflag) {
            ...
            Vector<ServerMap> serverMaps = this.dc.getServerMaps();
            
            if (commandMap.isGroup()) {
            	//send command to a group of servers
                if (this.dc.getGrpsList().containsKey(commandMap.getDestination())) {
                	GroupMap currGroupMap = this.dc.getGrpsList().get(commandMap.getDestination());
                    
                    Vector<String> serverGroupMembers = currGroupMap.getMembers();
                    ...
                    for (int i = 0; i < serverGroupMembers.size(); ++i) {
                        String currServerGroupMember = serverGroupMembers.elementAt(i);
            ...
            BinaryMsgFormat msgFormat = new BinaryMsgFormat("test");
            
            for (int i = 0; i < n; ++i) {
                synchronized (this) {
                	ServerMap currServerMap = serverMapsArr[i];
                    ...
                    Socket socket = currServerMap.getSocket();
                    if (socket == null) {
                        continue;
                    }
                    try {
