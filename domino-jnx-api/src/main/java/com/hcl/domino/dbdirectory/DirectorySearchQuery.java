...
/**
 * With this query you can scan for files and subdirectories in a directory
 * (either locally or on the server).<br>
 * <br>
 * Specify a formula argument to improve efficiency when processing a subset of
 * the notes in a database.<br>
 * <br>
 * In addition, the formula argument can be used to return computed "on-the-fly"
 * information.<br>
 * <br>
 * To do this, you specify that a value returned from a formula is to be stored
 * in a
 * temporary field of each note.<br>
 * <br>
 * This temporary field and its value is then accessible in the summary buffer
 * received by
 ...
 * <br>
 * In the action routine of NSFSearch, if you get a search match, look at the
...
public interface DirectorySearchQuery extends SearchQuery {
  ...
  public enum SearchFlag {
    ...
    public static short toBitMaskSearch1Flags(final Collection<SearchFlag> searchFlagSet) {
      int result = 0;
      if (searchFlagSet != null) {
        for (final SearchFlag currFlag : SearchFlag.values()) {
          if (SearchFlag.SEARCH1_FLAGS.contains(currFlag) && searchFlagSet.contains(currFlag)) {
    ...
    public static int toBitMaskStdFlagsInt(final Collection<SearchFlag> searchFlagSet) {
      int result = 0;
      if (searchFlagSet != null) {
        for (final SearchFlag currFlag : SearchFlag.values()) {
          if (!SearchFlag.SEARCH1_FLAGS.contains(currFlag) && searchFlagSet.contains(currFlag)) {
   ...
   * @throws DominoException thrown if an error occured, e.g. the formula could
   *                         not be evaluated
   */
   ...
   * Note: When performing the search, it might throw a
