...
public interface MimeEntity {
	...
	/**
	 * Retrieves the content of the entity as an {@link InputStream}. If the entity's content is
	 * encoded, this decodes it.
	 * 
	 * <p>This is equivalent to calling {@code getInputStream(true)}.</p>
	 * 
	 * @return an {@link Optional} describing a {@link InputStream} of the content of the entity,
	 *      decoded from storage format, or an empty one if this is a multipart entity
	 */
	...
	/**
	 * Retrieves the content of the entity as an {@link InputStream}. If the entity's content is
	 * encoded, this decodes it if {@code decode} is {@code true} and leaves it encoded if it is
	 * {@code false}.
	 * 
	 * <p>This is equivalent to calling {@code getInputStream(true)}.</p>
	 * 
	 ...
	 * @return an {@link Optional} describing a {@link InputStream} of the content of the entity,
	 *      optionally decoded from storage format, or an empty one if this is a multipart entity
	 */
	Optional<InputStream> getInputStream(boolean decode);
	
	/**
	 * Retrieves a {@link Reader} for the text content of this entity. If the entity is encoded,
	 * this decodes it. Additionally, this reader will obey any {@code charset} parameter in the
	 * entity's {@code Content-Type} header, if present.
	 * 
	 * @return an {@link Optional} describing a {@link Reader} for the content of this entity as text,
	 ...
	 * encoded, this decodes it.
	 * 
	 * @return an {@link Optional} describing the content of the entity as a {@link String}, or
	 *      an empty one if this is a multipart entity
	 */
	// TODO consider whether this should promise to handle any Content-Encoding, etc. headers
	Optional<String> getContentAsString();
	
	/**
	 * Sets the content of the MIME entity from the provided {@link InputStream}, applying
	 * the requested storage encoding.
	 * 
	 * <p>Note: the content of the stream should not be already encoded.</p>
	 * 
	 ...
	 * @throws ParseException if the {@code Content-Type} header is present and invalid
	 */
	default Optional<ContentType> getContentType() throws ParseException {
		List<MimeHeader> h = getHeaders("Content-Type"); //$NON-NLS-1$
		if(h != null && !h.isEmpty()) {
	 ...
	 * 
	 * @param preamble the text of the preamble to set, or {@code null} to clear an existing
	 * 		preamble
	 ...
	 * 
	 * @return an {@link Optional} describing the multipart boundary string, or an empty one
	 ...
	 * 
	 * <p>If this entity does not currently have a {@code multipart} content type, this
	 * method sets the content type to {@code multipart/mixed} and any existing content
	 * is removed.</p>
	 * 
	 ...
	 * @return the newly-created child entity
	 * @throws IllegalArgumentException if {@code sibling} is not a child of the current entity
	 */
	 ...
	 * 
	 * <p>If the provided entity already has a parent entity, it is removed from that
	 * tree.</p>
	 * 
	 * <p>If this entity does not currently have a {@code multipart} content type, this
	 * method sets the content type to {@code multipart/mixed} and any existing content
	 ...
	 */
	// TODO consider whether this makes sense to do - it's good conceptually, but the immediate
	//    C API calls for manipulating note-based MIME entities don't work with them in quite
	//    this way
	...
	/**
	 * Returns a mutable {@link Iterable} view of the children of this multipart entity's
	 * children.
	 * 
	 * <p>Calling {@link #createChildEntity(MimeEntity)} will invalidate any objects previously
	 * returned by this method.</p>
	 * 
	 * @return an {@link Optional} describing a mutable {@link Iterable}, or an empty one if
	 ...
	 * provided value.
	 * 
	 * <p>{@code value} is treated as the entire content of the header, and may contain
	 * parameters.</p>
	 * 
	 * @param headerName the name of the header to create
