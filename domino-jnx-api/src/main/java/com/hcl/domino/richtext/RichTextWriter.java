...
public interface RichTextWriter extends AutoCloseable {
	 ...
	 * @param textStyle text style
	 * @param fontStyle font style
	 * @param createParagraphOnLinebreak true to create a paragraph for each linebreak found in the text
	 * @return this writer
	 */
	 ...
	 * 
	 * <p>Note: this uses the structure of image-resource design elements and differs significantly
	 * from how inline images are handled. Use {@link #addImage(Path)} and the
	 * related methods to add images to a UI rich-text field.</p>
	 * 
	 ...
	 * @since 1.0.15
	 */
	default RichTextWriter addImageResource(Path file) throws IOException {
		long len = Files.size(file);
		try(InputStream is = Files.newInputStream(file)) {
			return addImageResource(is, len);
		}
	}
	 ...
	 * Adds the provided stream as an image resource into this rich text writer.
	 * 
	 * <p>If the length of the content is unknown, pass {@code -1} for the {@code length} parameter.
	 * This is inefficient, however, as it will cause the underlying implementation to read the
	 * entire stream at the start instead of as needed.</p>
	 * 
	 * <p>Note: this uses the structure of image-resource design elements and differs significantly
	 * from how inline images are handled. Use {@link #addImage(InputStream)} and the
	 * related methods to add images to a UI rich-text field.</p>
	 * 
	 ...
	 * 
	 * @param dbReplicaId Replica ID of the database that contains the document pointed to by the DocLink.
	 * @param viewUnid UNID of the {@link DominoCollection} that contains the document pointed to by the DocLink or null/empty string to create a database link
	 * @param docUNID UNID of the document pointed to by the DocLink or null/empty string to create a view/collection link
	 * @param comment This string appears when the DocLink is selected (clicked on).
	 * @return this writer
	 */
	 ...
	 * 
	 * <p>Note: this uses the structure of file-resource design elements and differs significantly
	 * from how attachments are handled. Use {@link #addAttachmentIcon(Attachment, String)} and the
	 * related methods to add attachments.</p>
	 * 
	 * @param file the file to write to the rich text buffer
	 * @return this writer
	 * @throws IOException if there is a problem reading the file data
	 ...
	 * <p>Note: this uses the structure of file-resource design elements and differs significantly
	 * from how attachments are handled. Use {@link #addAttachmentIcon(Attachment, String)} and the
	 * related methods to add attachments to a rich-text field.</p>
	 * 
	 * @param is the {@link InputStream} to write to the rich text buffer
	 * @param length the length of the file, if known; {@code -1} otherwise
	 * @return this writer
	 ...
	 * 
	 * <p>Note: this users the structure of script-library design elements and is not likely to be
	 * useful in UI rich text.</p>
	 * 
	 ...
	 * @param recordType a {@link RecordType} record type representing the record to create
	 * @param processor a {@link Consumer} to configure the record
	 * @return this writer
	 */
	 ...
	 * the provided function before appending it to the record stream.
	 * 
	 * <p>This method allows you to specify an additional amount of space to allocate for the structure's
	 * variable data, which will be accessible just after the final part of the structure.</p>
	 * 
	 * @param <T> the type of record to add
	 * @param recordClass a {@link Class} record representing {@code <T>}
	 ...
	 * @return this writer
	 */
	<T extends RichTextRecord<?>> RichTextWriter addRichTextRecord(Class<T> recordClass, int variableDataLength, Consumer<T> processor);
	...
	/**
	 * Creates a new {@link MemoryStructure} of the given class in memory, without appending it to the
