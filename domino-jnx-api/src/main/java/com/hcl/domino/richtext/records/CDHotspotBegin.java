...
public interface CDHotspotBegin extends RichTextRecord<WSIG> {
	...
	/**
	 * Sets the variable data portion of this structure to the provided string values. The structure must
	 * have been allocated with enough space to hold the LMBCS-encoded versions of both names and null
	 * terminators for each.
	 * 
	 * <p>This is for use when the hotspot type is {@link Type#FILE}.</p>
	 * 
	 * <p>This method also sets the {@code DataLength} property to the appropriate value.</p>
	 * 
	 ...
	 * @param appendNulls whether this method should append null bytes after each name
	 * @return this record
	 */
	default CDHotspotBegin setFileNames(String uniqueFileName, String displayFileName) {
		byte[] uniqueFileNameAttachment = uniqueFileName.getBytes(Charset.forName("LMBCS-native")); //$NON-NLS-1$
		byte[] fileNameToDisplayMem = displayFileName.getBytes(Charset.forName("LMBCS-native")); //$NON-NLS-1$
		setFileNamesRaw(uniqueFileNameAttachment, fileNameToDisplayMem, true);
		
		return this;
	}
	/**
	 * Sets the variable data portion of this structure to the provided LMBCS-encoded string values. The
	 * structure must have been allocated with enough space to hold both and, if specified, the added
	 * null values.
	 * 
	 * <p>This is for use when the hotspot type is {@link Type#FILE}.</p>
	 * 
	 ...
	 * @return this record
	 */
	default CDHotspotBegin setFileNamesRaw(byte[] uniqueFileName, byte[] displayFileName, boolean appendNulls) {
		int len = uniqueFileName.length + displayFileName.length + (appendNulls ? 2 : 0);
		resizeVariableData(len);
		setDataLength(len);
	...
	/**
	 * Retrieves the subform name or formula for a {@link Type#SUBFORM SUBFORM}-type hotspot.
	 * 
	...
	default String getSubformValue() {
		if(getHotspotType() != Type.SUBFORM) {
			throw new IllegalStateException(MessageFormat.format("Cannot retrieve the subform name for a hotspot of type {0}", getHotspotType()));
		}
		if(getFlags().contains(Flag.FORMULA)) {
			...
			return StructureSupport.extractStringValue(
				this,
				0,
				getDataLength()-1 // null terminated
			);
		}
	}
}
