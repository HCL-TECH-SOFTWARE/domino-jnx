...
public class RemoveAttachmentIconConversion implements IRichTextConversion {
	 ...
	 */
	private int findContentAfterHotspotEnd(List<RichTextRecord<?>> nav, int startPos) {
		if(nav.isEmpty()) {
			return -1;
		}
		
		ListIterator<RichTextRecord<?>> iter = nav.listIterator(startPos);
		while(iter.hasNext()) {
	...
	private int findBeginBeforeHotspot(List<RichTextRecord<?>> nav, int startPos) {
	 ...
	 */
	private int scanForBeginOfHotspot(List<RichTextRecord<?>> nav, int startPos, RichTextWriter copyToTarget) {
		if(nav.isEmpty()) {
			return -1;
		}
					...
					if (iter.hasNext()) {
						...
						if (record instanceof CDHotspotBegin) {
							if (((CDHotspotBegin) record).getHotspotType() == CDHotspotBegin.Type.FILE) {
								String uniqueFileName = ((CDHotspotBegin)record).getUniqueFileName();
	...
	public boolean isMatch(List<RichTextRecord<?>> nav) {
		int pos = findBeginBeforeHotspot(nav, 0);
		if (pos > -1) {
			return true;
		}
		else {
	...
	public void convert(List<RichTextRecord<?>> source, RichTextWriter target) {
		...
		if (!source.isEmpty()) {
			while (true) {
				...
				int nextBeginBeforeHotspot = scanForBeginOfHotspot(source, currPos, target);
				if (nextBeginBeforeHotspot > -1) {
					int contentAfterHotspotEnd = findContentAfterHotspotEnd(source, nextBeginBeforeHotspot);
