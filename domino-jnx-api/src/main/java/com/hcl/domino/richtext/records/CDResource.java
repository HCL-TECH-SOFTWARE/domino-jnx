...
public interface CDResource extends RichTextRecord<WSIG> {
	...
	enum ResourceClass implements INumberEnum<Short> {
		...
		;
		private final short value;
		ResourceClass(short value) { this.value = value; }
	...
	default CDResource setServerHint(String hint) {
	...
	default CDResource setFileHint(String hint) {
		return StructureSupport.writeStringValue(
			this,
			getServerHintLength(),
			getFileHintLength(),
	 ...
	 * 
	 * <p>Note: this only applies when {@link #getResourceType()} is {@link Type#NOTELINK}.</p>
	 * 
	 ...
	 * 
	 * <p>Note: this only applies when {@link #getResourceType()} is {@link Type#NAMEDELEMENT}.</p>
	 * 
	 * @return the name of the referenced element
	 * @throws IllegalStateException when the type is not {@link Type#NAMEDELEMENT} or if the value is a formula
	 */
	default String getNamedElement() {
		...
		return StructureSupport.extractStringValue(
			this,
			getServerHintLength() + getFileHintLength() + 8, // for replica ID
			getLength1()
		);
	}
	
	/**
	 * Retrieves the formula for the name of the element referenced by this resource.
	 * 
	 * <p>Note: this only applies when {@link #getResourceType()} is {@link Type#NAMEDELEMENT}.</p>
	 * 
	 * @return the name of the referenced element
