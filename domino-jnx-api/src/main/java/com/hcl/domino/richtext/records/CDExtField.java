...
public interface CDExtField extends RichTextRecord<WSIG> {
	...
	enum Flag2 implements INumberEnum<Integer> {
		...
		;
		private final int value;
		Flag2(int value) { this.value = value; }
	...
	enum HelperType implements INumberEnum<Short> {
		...
		HelperType(short value) { this.value = value; }
	...
	default CDExtField setEntryDBName(String name) {
		int viewNameLen = getEntryViewNameLength();
		byte[] lmbcs = name == null ? new byte[0] : name.getBytes(Charset.forName("LMBCS")); //$NON-NLS-1$
		setEntryDBNameLength(lmbcs.length);
		resizeVariableData(viewNameLen+lmbcs.length);
	...
	default String getEntryViewName() {
		int len = getEntryViewNameLength();
		if(len == 0) {
			return ""; //$NON-NLS-1$
		}
		int preLen = getEntryDBNameLength();
		ByteBuffer buf = getVariableData();
		buf.position(preLen);
		byte[] lmbcs = new byte[len];
		buf.get(lmbcs);
	...
	default CDExtField setEntryViewName(String name) {
		int dbNameLen = getEntryDBNameLength();
		byte[] lmbcs = name == null ? new byte[0] : name.getBytes(Charset.forName("LMBCS")); //$NON-NLS-1$
		setEntryViewNameLength(lmbcs.length);
		resizeVariableData(dbNameLen+lmbcs.length);
