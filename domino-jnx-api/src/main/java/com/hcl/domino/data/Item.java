...
public interface Item extends IAdaptable {
	 ...
	 * @param defaultValue default value to be returned of object property is not set
	 * @return return value
	 ...
	 * the list may contain only one element (e.g. for {@link ItemDataType#TYPE_TEXT}.
	 * <br>
	 * 
	 ...
	 * <li>{@link ItemDataType#TYPE_OBJECT} with the subtype Attachment (e.g. $File items) - List with {@link Attachment} object</li>
	 ...
	 * NAMES fields are used to store Domino usernames and these fields can be checked 
	 * by the server's Administration Process (adminp) to update if the username changes 
	 ...
	 * READERS fields are used to store Domino usernames and restrict read access to documents.
	 * These fields can be checked by the server's Administration Process (adminp) to update 
	 ...
	 * READWRITERS fields are used to store Domino usernames and restrict saving changes to documents 
	 ...
	 * If so, the developer will need to call {@link #setProtected(boolean) setProtected(false)} before making a change, and 
	 ...
	 * 
	 * TODO: Under what circumstances would this be true and under what circumstances would it be false?
	 ...
	 * the developer will need to call {@link #setProtected(boolean) setProtected(false)} before making a change, and 
	 * set it back to true afterwards.
	 * 
	 * <code>
	 * Item itm = doc.getFirstItem("Foo");
	 ...
	 * </code>
	 * 
	 * @param isProtected true if editor access or above is required to edit the Item
	 */
	 ...
	 * 
	 * @return a {@link DocumentProperties} object representing the document's meta properties
	 */
	 ...
	 * 
	 * @return an {@link Optional} describing a {@link MimeEntity} for the item content, or
	 *      an empty one if this is not a MIME item
	 */
	Optional<MimeEntity> getMimeEntity();
	
	/**
	 * This function converts the input {@link ItemDataType#TYPE_RFC822_TEXT} item in an open document
	 * to its pre-V5 equivalent; i.e. to {@link ItemDataType#TYPE_TEXT}, {@link ItemDataType#TYPE_TEXT_LIST},
	 * or {@link ItemDataType#TYPE_TIME}.<br>
	 * <br>
	 * For example, we convert the PostedDate {@link ItemDataType#TYPE_RFC822_TEXT} item to a
	 * {@link ItemDataType#TYPE_TIME} item.<br>
	 * <br>
	 * It does not update the Domino database; to update the database, call {@link Document#save()}.<br>
	 * 
