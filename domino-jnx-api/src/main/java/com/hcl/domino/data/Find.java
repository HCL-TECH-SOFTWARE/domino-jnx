 ...
 * to control how the
 * collection is searched for the key. The flags, {@link #PARTIAL}, {@link #CASE_INSENSITIVE},
...
public enum Find {
	...
	public static short toBitMask(Set<Find> findSet) {
		int result = 0;
		if (findSet!=null) {
	...
	public static int toBitMaskInt(Set<Find> findSet) {
		int result = 0;
		if (findSet!=null) {
			for (Find currFind : values()) {
				if (findSet.contains(currFind)) {
