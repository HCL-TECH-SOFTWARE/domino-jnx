...
public interface Document extends TypedAccess, IAdaptable {
	...
	/**
	 * Returns an instance of this document that implements the {@link AutoCloseableDocument}
	...
	/**
	 * Retrieves the note ID of this document. This value is specific to the NSF and is not unique
	 * across replicas.
	 * 
	 ...
	 * 
	 * <p>Note: by default, this value is derived from the document's UNID, but can be overridden
	 * by the presence of a {@code $Created} date/time item.</p>
	 * 
	 ...
	 * in the database.
	 * 
	 * @return the timestamp of the document last access; may be the minimum time value if
	 ...
	 * @return an {@link Optional} describing the {@link Item}, or an empty one if no such item exists
	 */
	 ...
	 * @return an {@link Optional} describing the attachment, or an empty one if no such
	 *      attachment exists
	 */
	 ...
	 * @param flags a collection of {@link ItemFlag} to apply to the item
	 * @param value the value of the item to store
	 * @param allowDataTypeChanges to append the item even if its type doesn't match the existing values
	 * @return this document
	 */
	...
	/**
	 * Retrieves the named rich-text item as an unmodifiable {@link List} of composite data records.
	 * 
	 * <p>This data may span multiple items of the same name, and all are included in the returned list.</p>
	 * 
	 * <p>Note: though presented as a {@link List}, some operations (in particular {@link List#size() size()})
	 * are particularly expensive. The most efficient way to traverse the result is via {@link List#stream()}
	 * or {@link List#listIterator()}.</p>
	 * 
	 * @param itemName the name of the rich text item to retrieve
	 * @return a {@link List} of {@link RichTextRecord} objects
	 */
	default RichTextRecordList getRichTextItem(String itemName) {
		return getRichTextItem(itemName, RecordType.Area.TYPE_COMPOSITE);
	}
	
	/**
	 * Retrieves the named rich text item as an unmodifiable {@link List} of composite data records, to be treated
	 ...
	 * 
	 * <p>Unlike {@link #attachFile(String, TemporalAccessor, TemporalAccessor, IAttachmentProducer)}, this
	 * method reads data from the named file on disk directly.</p>
	 * 
	 * @param filePathOnDisk a path to the file on disk
	 * @param uniqueFileNameInDoc a name for the attachment in the document
	 * @param compression the compression scheme to use when storing the attachment
	 ...
	 * Attaches a file to the document, not associated with a rich-text item.
	 * 
	 * <p>Unlike {@link #attachFile(String, String, Compression)}, this method allows for specifying file metadata
	 * and source binary data arbitrarily. It does not use compression. No temp
	 * file is created on disk.</p>
	 * 
	 * @param uniqueFileNameInDoc a name for the attachment in the document
	 * @param fileCreated the creation date for the stored file
	 * @param fileModified the modification date for the stored file
	 * @param producer a {@link IAttachmentProducer} implementation that provides the attachment data
	 * @return the newly-created document attachment
	 */
	 ...
	 * 
	 * <p>Note: this does not remove references to the document from rich-text items.</p>
	 * 
	 * @param uniqueFileNameInDoc the unique internal name of the attachment in the document
	 ...
	 * <br>
	 * It can encrypt a document in several ways -- by using the Domino public key of the caller,
	 * by using specified secret encryption keys stored in the caller's ID, or by using the
	 * Domino public keys of specified users, if the document does not have any mime parts.<br>
	 * <br>
	 ...
	 * Each field is either a {@link ItemDataType#TYPE_TEXT} or {@link ItemDataType#TYPE_TEXT_LIST} field.<br>
	 * <br>
	 ...
	 * This feature is intended to allow a group to encrypt some of the documents in a single
	 ...
	 * The user can then decrypt the document with the private key in the user's ID.<br>
	 * This feature provides a way to encrypt documents, such as mail documents, for another user.<br>
	 * <br>
	 * The document must contain at least one encryption enabled item (an item with the {@link ItemFlag#ENCRYPTED} flag set)
	 ...
	 * 
	 * <p>This is equivalent to calling {@code delete(false)}.</p>
	 */
	void delete();
	 ...
	 * 
	 * <p>This is equivalent to calling {@code save(false)}.</p>
	 */
	void save();
	 ...
	 * Writes in-memory changes to a document to the database.
	 * 
	 * @param force whether to save the document even if another user has modified or deleted it
	 */
	void save(boolean force);
	
	/**
	 * 
	 * @return a {@link List} of user names that currently have a lock on the document
	 */
	 ...
	 * 
	 * <p>Note: due to the lower-level API, this is likely to be more efficient than
	 * {@link #getResponses() retrieving the response document collection} and checking its count.</p>
	 * 
	 ...
	 * 
	 * <p>In order for this method to return the response table, this document must have been opened
	 * with the {@link OpenDocumentMode#LOAD_RESPONSES} flag.</p>
	 * 
	...
	/**
	 * Returns TRUE if the given note contains either {@link ItemDataType#TYPE_RFC822_TEXT}
	...
	/**
	 * The function returns TRUE if the given note contains any {@link ItemDataType#TYPE_MIME_PART} items.
	 * 
	...
	/**
	 * Returns TRUE if the given note contains any {@link ItemDataType#TYPE_COMPOSITE} items.
	 * 
	 ...
	 */
	@Override boolean hasItem(String itemName);
	 ...
	 * This is the case if the database containing the note has replication settings set to
	 ...
	 * created via {@link Database#createDocument(Set)} with {@link CreateFlags#HIDE_FROM_VIEWS}
	 * 
	 ...
	 * <br>
	 * If a callback routine is supplied and one of the validation formulas fails, the
	 * error callback routine is called.<br>
	 * If the {@code continueOnError} flag is set, error processing for the field is skipped;<br>
	...
	/**
	 * Possible validation phases for {@link Document#computeWithForm(boolean, ComputeWithFormCallback)}
	 */
	...
	public enum ComputeWithFormAction {
		/**
		 * End all processing by {@link Document#computeWithForm(boolean, ComputeWithFormCallback)}
		...
		ComputeWithFormAction(short val) {
			this.actionVal = val;
		}
	 ...
	 * 
	 * @param userName username in abbreviated or canonical format, if null, we use {@link DominoClient#getEffectiveUserName()}
	 * @return true if unread
	 */
	boolean isUnread(String userName);

	/**
	 * Checks if the document is unread for the user returned by {@link DominoClient#getEffectiveUserName()}.<br>
	 * Uses {@link Database#isDocumentUnread(String, int)} internally.
	 * 
	 * @return true if unread
	 */
	 ...
	 * @return true if rich text has been updated, false if all conversion classes returned {@link IRichTextConversion#isMatch(List)} as false
	 */
	 ...
	 * 
	 * <p>This differs from calling {@link #get} with `String.class` as the type in that it uses
	 * an internal Domino routine to convert values to text.</p>
	 * 
	 * @param itemName the name of the item to retrieve
	 ...
	 * 
	 * <p>This is equivalent to calling {@code get("$REF", String.class, "")}.</p>
	 * 
	 * @return the UNID of the document's parent, or an empty string if the document has no parent
	 * @since 1.0.7
	 */
	default String getParentDocumentUNID() {
		return get("$REF", String.class, ""); //$NON-NLS-1$ //$NON-NLS-2$
	}
	 ...
	 * 
	 * @param itemName item name
	 * @param value string containing the text you wish to add as the NEXT entry in the existing text list in the document
	 * @param allowDuplicates A boolean containing a flag indicating whether or not to allow duplicate entries in the text list. {@code true} - If you wish to allow them and {@code false} - If you do not.
	 */
