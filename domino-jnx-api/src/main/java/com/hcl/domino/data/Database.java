...
public interface Database extends IAdaptable, AutoCloseable, DominoClientDescendant {
	 ...
	 * 
	 * <p>If the database is on a remote server, this path will be relative to the server's data
	 * directory.</p>
	 * 
	 * <p>If the database is local, this will be an absolute local file path.</p>
	 * 
	 ...
	 * @return an {@link Optional} describing the requested collection, or an empty one if
	...
	public enum OpenDocumentMode {
		 ...
		 * Warning: documents opened with OPEN_NOOBJECTS then subsequently updated loose all
	 ...
	 * 
	 * @param noteId the ID of the document to retrieve
	 * @return an {@link Optional} describing the soft-deleted document matching that ID, or
	 *      an empty one if there is no such document
	 */
	 ...
	 * @return an {@link Optional} describing the soft-deleted document matching that UNID, or
	 *      an empty one if there is no such document
	 */
	 ...
	 * (via {@link Document#setPrimaryKey(String, String)}, hashed by their category value
	 * and object id
	 * 
	...
	/**
	 * Returns the note id of all documents with <code>category</code> in the assigned primary key
	 * (via {@link Document#setPrimaryKey(String, String)}, hashed by their category value
	 * and object id
	 * 
	 ...
	 * @return an {@link Optional} describing the existing or new profile document, or an
	 ...
	 * @return replication results
	 * @param progressListener optional listener to get notified about replication progress
	 */
	NotesReplicationStats replicate(String serverName, Set<ReplicateOption> options, int timeLimitMin, ReplicationStateListener progressListener);
	 ...
	 * 
	 * @return an {@link Optional} describing the last index time, or an empty one if the
	 ...
	 * 
	 * <p>Note: this may be slightly more efficient than {@link #getLocalEncryptionInfo}.</p>
	 * 
	 * @return {@code true} if the database is encrypted on disk; {@code false} otherwise
	 */
	 ...
	 * deleted.
	 * 
	 * <p>Calling this method will cause all future invocations of methods on this
	 * object to fail.</p>
	 */
	@Override void close();
	 ...
	 * The data returned by this method is the note id the UNID of the document, the sequence number
	 ...
	 * 
	 * @param agentName agent name
	 * @return an {@link Optional} describing the agent's saved data, or an empty one if no
	 ...
	 * @param maxResults Maximum number of documents to return (max. 65535). Use 0 to return the maximum number of results for the search
	 * @param options search options
	 * @param filterIds optional ID table to further refine the search.  Use null if this is not required.
	 ...
	 * @param count number of documents to return for the paged result (max. 65535), set to 0 to return all results
	 ...
	 * @param filter optional filter IDTable to evaluate the formula only against selected documents
	 * @param searchFlags flags to control the search behavior
	 * @param since optional start date/time to run incremental searches, e.g. the value {@link IDTable#getDateTime()} of the returned {@link FormulaQueryResult#getNoteIds()}
	 ...
	 * 
	 * @return query result, use {@link DocumentSummaryQueryResult#computeValues(Map)} to compute document values on the fly and {@link DocumentSummaryQueryResult#sort(DominoCollection)} to project the document note ids onto a {@link DominoCollection} to get them back sorted via {@link DocumentSummaryQueryResult#collectEntries(int, int)}
	 */
	...
	public enum DocFlags {
		...
		NoPurgeStatus,
		/** if {@link SearchFlag#NOTIFYDELETIONS}: note is soft deleted; NoteClass &amp; {@link DocumentClass#NOTIFYDELETION} also on (off for hard delete) */
		SoftDeleted,
	 ...
	 * <br>
	 * Use the date/time value returned by {@link IDTable#getDateTime()} as <code>since</code>
	 * parameter of {@link #getModifiedNoteIds(Set, TemporalAccessor, boolean)} to incrementally
	 * read note id changes.<br>
	 * <br>
	 * This function is just a convenience function and calls {@link #getModifiedNoteIds(Set, TemporalAccessor, boolean)}
	 * with a since value of NULL.
	 * 
	 * @param docClasses the appropriate {@link DocumentClass} mask for the documents you wish to select. Symbols can be OR'ed to obtain the desired Note classes in the resulting ID Table.  
	...
	/**
	 * This function returns an {@link IDTable} of Note IDs of documents which have been modified in some way
	 * from the given starting time until "now".<br>
	 * The ending date/time is returned in {@link IDTable#getDateTime()}, so that this
	 * function can be performed incrementally.<br>
	 * <br>
	 * The returned IDTable also contains the note IDs of deleted documents. These note IDs are bitwise OR'ed
	 * with the special flag {@link IDTable#NOTEID_FLAG_DELETED}.<br>
	 * 
	 * @param docClasses the appropriate {@link DocumentClass} mask for the documents you wish to select. Symbols can be OR'ed to obtain the desired Note classes in the resulting ID Table.  
	 * @param since A date/time value containing the starting date used when selecting notes to be added to the ID Table built by this function. Use null to get all note ids.
	 * @param includeDeletions true to include the note IDs of deleted docs
	 * @return IDTable
	 */
	IDTable getModifiedNoteIds(Set<DocumentClass> docClasses, TemporalAccessor since, boolean includeDeletions);

	public enum CollectionType { View, Folder, Both }
	 ...
	 * <br>
	 * This is the number of documents plus the number of categories (if any) in the folder.<br>
	 * <br>
	...
	public enum ModifiedNotesInfoFlags implements INumberEnum<Integer> {
		...
		ModifiedNotesInfoFlags(Integer value) {
			m_value = value;
		}
	 ...
	 * 
	 * @param docClasses document classes to scan, e.g. {@link DocumentClass#DATA}
	 * @param infoRequested controls what to return and how to filter
	 * @param sinceParam optional for incremental scans, use {@link ModifiedNoteInfos#getUntil()} of a previous search
	 ...
	 * <br>
	 * The argument {@code createIfNotAvailable} controls what action is to be performed
	 * if there is no list of unread notes for the specified user in the database.<br>
	 * <br>
	 * If no list is found and this flag is set to {@code false}, the method will return {@code null}.<br>
	 * if this flag is set to {code true}, the list of unread documents will be created and all
	 * documents in the database will be added to the list.<br>
	 * <br>
	 * No coordination is performed between different users of the same database.<br>
	 * <br>
	 * If an application obtains a list of unread documents while another user is modifying the
	 * list, the changes made may not be visible to the application.<br>
	 * <br>
	 * Unread marks for each user are stored in the client desktop.dsk file and in the database.<br>
	 * <br>
	 * When a user closes a database (either through the Notes user interface or through an API program),
	 * the unread marks in the desktop.dsk file and in the database are synchronized so that
	 * they match.<br>
	 * Unread marks are not replicated when a database is replicated.<br>
	 * <br>
	 * Instead, when a user opens a replica of a database, the unread marks from the desktop.dsk
	 * file propagates to the replica database.<br>
	 * 
	 * @param userName user for which to check unread marks (abbreviated or canonical format); use {@code null} for current {@link DominoClient} user
	 * @param createIfNotAvailable {code true}: If the unread list for this user cannot be found on disk, return all note IDs. {@code false}: If the list cannot be found, return null
	 * @param updateUnread {@code true} to update unread marks, {@code false} to not update unread marks.
	 * @return an {@link Optional} describing the table of unread documents, or an empty one if there is no table
	 */
	 ...
	 * 
	 * @param userName user for which to update unread marks (abbreviated or canonical format); use {@code null} for current {@link DominoClient} user
	 * @param noteIdToMarkRead note ids to mark read (=remove from the unread table)
	...
	/**
	 * Convenience method to look up the roles for a user from the database {@link Acl}.<br>
	 * Use {@link Acl#lookupAccess(UserNamesList)} if you need other access information
	 ...
	 * 
	 * @param userName name if user in abbreviated or canonical format; if null, we use {@link DominoClient#getEffectiveUserName()}
	 ...
	 * the database.<br>
	 * The deleted document may be of any {@link DocumentClass}. The active user ID must have
	 ...
	 * 
	 * @param retDataModified will be filled with the last modified date/time of data documents
	 * @param retNonDataModified will be filled with the last modified date/time of non-data (design) documents
	 */
	void getModifiedTime(Ref<DominoDateTime> retDataModified, Ref<DominoDateTime> retNonDataModified);

	/**
	 * This function generates a new Originator ID (OID) used to uniquely identify a document.<br>
	 * <br>
	 * Use this function when you already have a document open and wish to create a totally new document
	 * with the same items as the open document.<br>
	 * <br>
	 * You do not need this method when creating a new note from scratch using {@link #createDocument()},
	 ...
	 * 
	 * <p>This can be useful for additional safely when working with the same database across
	 * multiple threads.</p>
	 * 
	 ...
	 * 
	 * @return item definition table with item names as keys (case-insensitive and sorted) and item type as value
	 */
	...
	interface EncryptionInfo {
		/**
		 * @return an {@link Optional} describing the current encryption state of the database, or
		 *      an empty one if the information could not be determined 
		 */
		Optional<DatabaseEncryptionState> getState();
		/**
		 * @return an {@link Optional} describing the current encryption strength of the database, or
		 *      an empty one if the information could not be determined
		 */
	...
	interface NSFVersionInfo {
		 ...
		 * The major verson number of a database is the same as the ODS version number that
		 * is displayed in the Notes Client File/Database/Properties/second information tab.
		 *  
	 ...
	 * Each release of Domino or Notes software can access databases that have a major version
	 * number that is less than or equal to a particular value associated with that release.<br>
	 * <br>
	 * A Domino database that has a major version number that is greater than the major version
	 * number associated with a particular Domino or Notes release cannot be accessed by that release.<br>
	 * <br>
	 * The following table shows which major version numbers can be accessed by particular
	 * releases of Domino or Notes:<br>
	 * <br>
	 * <table>
	 * <tr><th>Domino or Notes Software Releases</th><th>Major Version Numbers That Can Be Accessed</th></tr>
	 * <tr><td>1.x</td><td>16</td></tr>
	 * <tr><td>2.x</td><td>16</td></tr>
	 * <tr><td>3.x</td><td>17</td></tr>
	 * <tr><td>4.0, 4.1, 4.5.x, 4.6.x</td><td>20 or less</td></tr>
	 * <tr><td>5.0 - 5.0.12, 6 - 6.0.3, 6.5, 7.0</td><td>43 or less</td></tr>
	 * <tr><td>9.0</td><td>52</td></tr>
	 * <tr><td>10.x - 11.x</td><td>53</td></tr>
	 * <tr><td>12.0.0</td><td>54</td></tr>
	 * </table>
	 * 
	 * @return ODS info
	 */
	...
	/**
	 * This function returns a {@link BuildVersionInfo} object which contains all types of
