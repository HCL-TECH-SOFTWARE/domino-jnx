...
public interface ViewFormatConstants {
	...
	byte VIEW_STYLE_TABLE = VIEW_CLASS_TABLE;
	byte VIEW_STYLE_DAY = (VIEW_CLASS_CALENDAR + 0);
	byte VIEW_STYLE_WEEK = (VIEW_CLASS_CALENDAR + 1);
	byte VIEW_STYLE_MONTH = (VIEW_CLASS_CALENDAR + 0);
	...
	/**
	 * Unofficial derived mask to include only the actual flag bits from VIEW_COLUMN_FORMAT.Flags2
	 */
	short VCF2_MASK_FLAGS = ~(VCF2_M_DisplayAlignment | VCF2_M_SubtotalCode | VCF2_M_HeaderAlignment | VCF2_M_DisplayReadingOrder | VCF2_M_HeaderReadingOrder);
