...
public interface NotesConstants extends ViewFormatConstants {
	...
	int MAXWORDAsInt = (int) (MAXWORD & 0xffff);
...
*/
	...
	short NOTE_CLASS_DATA = NOTE_CLASS_DOCUMENT;
	...
	short NOTE_CLASS_NOTIFYDELETION	= NOTE_CLASS_DEFAULT;
	/** all note types */
	...
	short NOTE_CLASS_SINGLE_INSTANCE =(
			NOTE_CLASS_DESIGN |
			NOTE_CLASS_ACL |
			NOTE_CLASS_INFO |
			NOTE_CLASS_ICON |
	...
	int SIGNAL_ANY_CONFLICT	= (SIGNAL_DEFN_ITEM_MODIFIED | SIGNAL_VIEW_ITEM_MODIFIED | SIGNAL_INDEX_MODIFIED | SIGNAL_UNREADLIST_MODIFIED | SIGNAL_DATABASE_MODIFIED);
	...
	int SIGNAL_ANY_NONDATA_CONFLICT	= (SIGNAL_DEFN_ITEM_MODIFIED | SIGNAL_VIEW_ITEM_MODIFIED | SIGNAL_INDEX_MODIFIED | SIGNAL_UNREADLIST_MODIFIED);
	...
	long RRV_DELETED = NOTEID_RESERVED;
	...
	/** obsolete synonym */
	int SEARCH_INCLUDE_DELETED = SEARCH_ALL_VERSIONS;
	...
	int SEARCH1_SELECT_NAMED_GHOSTS = (0x00000001 | SEARCH1_SIGNATURE);
	 ...
	 */
	int SEARCH1_PROFILE_DOCS = (0X00000002 | SEARCH1_SIGNATURE);

	/**
	 * Skim off notes whose summary buffer can't be generated because its size
	 * is too big.
	 */
	int SEARCH1_SKIM_SUMMARY_BUFFER_TOO_BIG = (0x00000004 | SEARCH1_SIGNATURE);
	int SEARCH1_RETURN_THREAD_UNID_ARRAY = (0x00000008 | SEARCH1_SIGNATURE);
	int SEARCH1_RETURN_TUA = SEARCH1_RETURN_THREAD_UNID_ARRAY;

	/**
	 * flag for reporting noaccess in case of reader's field at the doc level
	 */
	int SEARCH1_REPORT_NOACCESS = (0x000000010 | SEARCH1_SIGNATURE);
	/** Search "Truncated" documents */
	int SEARCH1_ONLY_ABSTRACTS = (0x000000020 | SEARCH1_SIGNATURE);
	/**
	 ...
	 * corrupt documents.
	 */
	int SEARCH1_FIXUP_PURGED = (0x000000040 | SEARCH1_SIGNATURE);

	/** used to indicate large summary buckets is supported by caller */
	int SEARCH1_LARGE_BUCKETS = (0x00000080 | SEARCH1_SIGNATURE);
	/** used to indicate to return SEARCH_MATCH_LARGE &amp; ilk */
	int SEARCH1_RET_LARGE_DATA = (0x00000100 | SEARCH1_SIGNATURE);
	...
	short PABFLAG_HIDEBITS = (short) ((PABFLAG_HIDE_RO | PABFLAG_HIDE_RW | PABFLAG_HIDE_CO | PABFLAG_HIDE_PR | PABFLAG_HIDE_PV | PABFLAG_HIDE_PVE | PABFLAG_HIDE_IF | PABFLAG_HIDE_NOTES) & 0xffff);

	short TABLE_PABFLAGS = (short) (( PABFLAG_KEEP_TOGETHER | PABFLAG_KEEP_WITH_NEXT) & 0xffff);
	...
	short PABFLAG2_HIDEBITS = PABFLAG2_HIDE_WEB;

	short PABFLAG2_CHECKLIST = (short) ((PABFLAG2_UNCHECKEDLIST | PABFLAG2_CHECKEDLIST)& 0xffff);

	short PABFLAG2_MARGIN_DEFAULTS_MASK = (short) (( PABFLAG2_LM_DEFAULT
					| PABFLAG2_RM_DEFAULT
					| PABFLAG2_FLLM_DEFAULT	) & 0xffff);

	short PABFLAG2_MARGIN_STYLES_MASK = (short) (( PABFLAG2_LM_OFFSET
				| PABFLAG2_LM_PERCENT
	...
	short PABFLAG2_MARGIN_MASK = (short) (( PABFLAG2_MARGIN_STYLES_MASK | PABFLAG2_MARGIN_DEFAULTS_MASK ) & 0xffff);

	short PABFLAG2_ROMANUPPERLIST = (short) ((PABFLAG2_CHECKEDLIST | PABFLAG2_CIRCLELIST) & 0xffff);
	short PABFLAG2_ROMANLOWERLIST = (short) ((PABFLAG2_CHECKEDLIST | PABFLAG2_SQUARELIST) & 0xffff);
	short PABFLAG2_ALPHAUPPERLIST = (short) ((PABFLAG2_SQUARELIST | PABFLAG2_CIRCLELIST) & 0xffff);
	short PABFLAG2_ALPHALOWERLIST = (short) ((PABFLAG2_CHECKEDLIST | PABFLAG2_SQUARELIST | PABFLAG2_CIRCLELIST) & 0xffff);
	...
	int OPEN_RAW_MIME = (OPEN_RAW_RFC822_TEXT | OPEN_RAW_MIME_PART);
	
	/* Message queue flags and constants */
	int MAXONESEGSIZE = 0xffff - 1-128;
	int MQ_MAX_MSGSIZE = MAXONESEGSIZE - 0x50;
	...
	/** Replicator */
	String REPL_QUEUE_NAME = TASK_QUEUE_PREFIX + "REPLICATOR"; //$NON-NLS-1$
	/** Mail Router */
	String ROUTER_QUEUE_NAME	= TASK_QUEUE_PREFIX + "ROUTER"; //$NON-NLS-1$
	/** Index views &amp; full text process */
	String UPDATE_QUEUE_NAME = TASK_QUEUE_PREFIX + "INDEXER"; //$NON-NLS-1$
	/** Login Process */
	String LOGIN_QUEUE_NAME = TASK_QUEUE_PREFIX + "LOGIN"; //$NON-NLS-1$
	/** Event process */
	String EVENT_QUEUE_NAME = TASK_QUEUE_PREFIX + "EVENT"; //$NON-NLS-1$
	/** Report process */
	String REPORT_QUEUE_NAME = TASK_QUEUE_PREFIX + "REPORTER"; //$NON-NLS-1$
	/** Cluster Replicator */
	String CLREPL_QUEUE_NAME = TASK_QUEUE_PREFIX + "CLREPL"; //$NON-NLS-1$
	/** Fixup */
	String FIXUP_QUEUE_NAME = TASK_QUEUE_PREFIX + "FIXUP"; //$NON-NLS-1$
	/** Collector*/
	String COLLECT_QUEUE_NAME = TASK_QUEUE_PREFIX + "COLLECTOR"; //$NON-NLS-1$
	/** NOI Process */
	String NOI_QUEUE_NAME = TASK_QUEUE_PREFIX + "DIIOP"; //$NON-NLS-1$
	/** Alarms Cache daemon */
	String ALARM_QUEUE_NAME = TASK_QUEUE_PREFIX + "ALARMS"; //$NON-NLS-1$
	/** Monitor */
	String MONITOR_QUEUE_NAME = TASK_QUEUE_PREFIX + "MONITOR"; //$NON-NLS-1$
	/** Monitor */
	String MONALARM_QUEUE_NAME = TASK_QUEUE_PREFIX + "MONITORALARM"; //$NON-NLS-1$
	/** Admin Panel Daemon (Request Queue) */
	String APDAEMON_REQ_QUEUE = TASK_QUEUE_PREFIX + "APDAEMONREQ"; //$NON-NLS-1$
	/** Admin Panel Daemon (File Response Queue) */
	String APDAEMON_FILERES_QUEUE = TASK_QUEUE_PREFIX + "APDAEMONFILERESPONSE"; //$NON-NLS-1$
	/** Admin Panel Daemon (Server Response Queue) */
	String APDAEMON_FILEREQ_QUEUE = TASK_QUEUE_PREFIX + "APDAEMONFILEREQUEST"; //$NON-NLS-1$
	/** bktasks */
	String BKTASKS_QUEUE_NAME = TASK_QUEUE_PREFIX + "BKTASKS"; //$NON-NLS-1$
	/** Red Zone Interface to Collector */
	String RZINTER_QUEUE_NAME = TASK_QUEUE_PREFIX + "RZINTER"; //$NON-NLS-1$
	/** Red Zone Extra MQ */
	String RZEXTRA_QUEUE_NAME = TASK_QUEUE_PREFIX + "RZEXTRA"; //$NON-NLS-1$
	/** Red Zone Background MQ */
	String RZBG_QUEUE_NAME = TASK_QUEUE_PREFIX + "RZBG"; //$NON-NLS-1$
	/** Red Zone Background Extra MQ */
	String RZBGEXTRA_QUEUE_NAME = TASK_QUEUE_PREFIX + "RZBGEXTRA"; //$NON-NLS-1$
	/** Monitor */
	String REALTIME_STATS_QUEUE_NAME = TASK_QUEUE_PREFIX + "REALTIME"; //$NON-NLS-1$
	/** Runjava (used by ISpy) */
	String RUNJAVA_QUEUE_NAME = TASK_QUEUE_PREFIX + "RUNJAVA"; //$NON-NLS-1$
	/** Runjava (used by ISpy) */
	String STATS_QUEUE_NAME = TASK_QUEUE_PREFIX + "STATS"; //$NON-NLS-1$
	/** Runjava (used by ISpy) */
	String LOG_SEARCH_QUEUE_NAME = TASK_QUEUE_PREFIX + "LOGSEARCH"; //$NON-NLS-1$
	/** Event process */
	String DAEMON_EVENT_QUEUE_NAME = TASK_QUEUE_PREFIX + "DAEMONEVENT"; //$NON-NLS-1$
	/** Collector*/
	String DAEMON_COLLECT_QUEUE_NAME = TASK_QUEUE_PREFIX + "DAEMONCOLLECTOR"; //$NON-NLS-1$
	/** Dircat */
	String DIRCAT_QUEUE_NAME = TASK_QUEUE_PREFIX + "DIRCAT"; //$NON-NLS-1$
	...
	int MIME_STREAM_INCLUDE_HEADERS = (MIME_STREAM_MIME_INCLUDE_HEADERS|MIME_STREAM_RFC2822_INCLUDE_HEADERS);
	...
	int MIME_STREAM_ITEMIZE_FULL = (MIME_STREAM_ITEMIZE_HEADERS|MIME_STREAM_ITEMIZE_BODY);
	...
	int DESIGN_NAME_MAX = ((DESIGN_LEVEL_MAX+1)*DESIGN_LEVELS);

	/** Forms can cascade a level */
	int DESIGN_FORM_MAX = DESIGN_NAME_MAX;
	/** Views can cascade a level */
	int DESIGN_VIEW_MAX = DESIGN_NAME_MAX;
	/** Macros can cascade a level */
	int DESIGN_MACRO_MAX = DESIGN_NAME_MAX;
	/** Fields cannot cascade */
	int DESIGN_FIELD_MAX = DESIGN_LEVEL_MAX+1;
	...
	/** Same as for views */
	int DESIGN_FOLDER_MAX	= DESIGN_VIEW_MAX;
	/** Same as for views */
	int DESIGN_FOLDER_MAX_NAME = DESIGN_LEVEL_MAX;
	...
	short DGN_LISTBOX = (DGN_STRIPUNDERS|DGN_SKIPSYNONYMS|DGN_SKIPBLANK);
	short DGN_MENU = (DGN_SKIPSYNONYMS|DGN_SKIPBLANK);
...
/* 0x00010000-0x8000000 WILL NOT BE HONORED BY V3 SERVERS, BECAUSE V3 ONLY LOOKS AT THE FIRST 16 BITS! */
	...
	int REPL_OPTION_ALL_FILES = (REPL_OPTION_ALL_DBS | REPL_OPTION_ALL_NTFS);
	...
	short MIME_HEADER_MAP_UNKNOWN = MIME_HEADER_MAP_EMAIL;
	...
	// Next 3 defines are commented out when added to V901FP7 for VOP, while they are not commented in V902.
	...
	// Collision in SaaS with DB_GET_MODIFIED_NOTES_INFO_SEQ_TIME moved to 0x00040000
	...
	short CWF_COMPUTED_FORMULA_LOAD = CWF_COMPUTED_FORMULA;
	...
	String MAIL_HTTPPASSWORD_ITEM = HTTP_PASSWORD_ITEM;
	 ...
	 * 
	 * <p>(Note: a Names value of "" must also be specified)</p>
	 */
	short NAME_LOOKUP_ALL = 0x0001;
	/**
	 * Only look in first names database containing desired namespace (view) for specified names
	 * rather than searching other names databases if name was not found.  Note that this may not
	 * necessarily be the first names database in the search path - just the first one containing
	 * the desired view
	 */
	...
	int MAX_CLIENT_AUTH_SIZE = (KEYBITS_256/8) + AES_BLOCK_SIZE;
	int PARTIAL_DH_TLV_SIZE = (KEYBITS_2048/8) + BSAFE_DIGEST_SIZE;
	...
	public int PROCRES_CREATE_VIEW = 0x00000001;
	/** Output of results processing is a JSON stream */
	public int PROCRES_JSON_OUTPUT = 0x00000002;
	/** Output of results processing is a SEARCH_MATCH || ITEM_TABLE summary stream*/
	public int PROCRES_SUMMARY_OUTPUT = 0x00000004;
	
	/* Flags governing JSON output */
	
	/** return UNID(s) with each result, default is NoteID(s) */
	public int PROCRES_RETURN_UNID = 0X00000008;
	/** return the replicaID of the database of the entry */
	public int PROCRES_RETURN_REPLICAID = 0x00000010;
	
	/* default is the index into the RESULTS_INFO_LIST */
	
	/** return a root element for each returned entry */
	public int PROCRES_RETURN_MULTIPLE_ROOTS = 0x00000020;
	public int PROCRES_STREAMED_OUTPUT = PROCRES_JSON_OUTPUT | PROCRES_SUMMARY_OUTPUT;
	/** internal flag to indicate category processing */
	public int PROCRES_HAS_CATEGORIZED_KEYS = 0x00000040;
	/** For formatting JSON arrays (from the outside) */
	public int PROCRES_JSON_PREPEND_COMMA = 0x00000080;
	/** If TYPE_ERROR occurs, drop the document from results with no error */
	public int PROCRES_IGNORE_TYPE_ERROR = 0x00000100;

	public int MAX_CMD_VALLEN = MAXSPRINTF + 1; // 256 + null term
	...
	public enum QUEP_LISTTYPE {
		...
		private QUEP_LISTTYPE(int value) {
			m_value = value;
		}
	...
	public enum DESIGN_COMPONENT_ATTR {
		...
		private DESIGN_COMPONENT_ATTR(int value) {
			m_value = value;
		}
	...
	public enum TypeOfProcessing{
		...
		private TypeOfProcessing(int value) {
			m_value = value;
		}
	...
	public enum EProvidingAuthor{
		...
		private EProvidingAuthor(int value) {
			m_value = value;
		}
	...
	public enum EUsePABsAdminServer{
		...
		private EUsePABsAdminServer(int value) {
			m_value = value;
		}
	...
	public enum EmptyFieldMeans {
		...
		private EmptyFieldMeans(int value) {
			m_value = value;
		}
	...
	public enum TypeOfLookup{
		...
		private TypeOfLookup(int value) {
			m_value = value;
		}
	...
	public enum DesignUpdateReset { 
		...
		private DesignUpdateReset(int value) {
			m_value = value;
		}
	...
	String AdminpSaaSPullMailfile = "182"; //$NON-NLS-1$  /* FAP added to aid in merge conflict resolution with fixes 217521, 214942, 218323 */
	String AdminpReplaceSaaSMailFileFields = "183"; //$NON-NLS-1$  /* FAP added to aid in merge conflict resolution with fixes 217521, 214942, 218323 */
	String AdminpDeletePersonInNABTimed = "184"; //$NON-NLS-1$  /* FAP added to aid in merge conflict resolution with fixes 217521, 214942, 218323 */
	...
	String ADMINP_ORIGINATING_UNID = ADMINP_ORIGINATING_REQUEST_UNID;
	...
	*/
	...
	String[] lpchMoveMailFile = {
			AdminpCheckMailServersAccess,
			AdminpCreateNewMailFileReplica,
			AdminpAddNewMailFileFields,
			AdminpMonitorNewMailFileFields,
			AdminpReplaceMailFileFields,
			AdminpLastPushToNewMailServer,		/* Terminator check on the admin server */
			ListSeperator,
			AdminpGetMailFileInfo,
			AdminpRequestDeleteMailFile,
			AdminpDeleteMailFile,				/* TERMINATOR SHOULD BE HERE!!! WILL FIX LATER */
			AdminpApproveMailFileInfo,			/* No purge for this is handled by the template */

	};

	String[] lpchMoveRoamingFiles = {
			AdminpCheckRoamingServerAccess,
			AdminpCreateRoamingReplicas,
			AdminpReplaceRoamingServerField,
			AdminpLastPushToNewRoamingServer,	/* Terminator check on the admin server */
			ListSeperator,
			AdminpGetFileInfo,
			AdminpRequestDeleteFile,
			AdminpDeleteFile,					/* TERMINATOR SHOULD BE HERE!!! WILL FIX LATER */
			AdminpApproveFileInfo				/* No purge for this is handled by the template */
	};
	...
	String[] lpchDeleteGroupNab = {
			AdminpDeleteGroupInNAB,
			AdminpDeleteInPersonDocs
	};
	String[] lpchCommonXNoFilesSupplied = {
			AdminpDeleteInTheACL,
			AdminpDeleteInReadersAuthors	/* Terminator check on admin server */
	};
	...
	String[] lpchCommonXForDeleteUser = {
			...
			ListSeperator,
			AdminpGetMailFileInfo,
			AdminpApproveMailFileInfo,
			AdminpRequestDeleteMailFile,
			AdminpDeleteMailFile
	};
	...
	String[] lpchMoveCertifier = {
			AdminpInitiateNABChange,
			AdminpRenameUserInNAB
	};
	String[] lpchRenameUser = {
			AdminpRenameUserInNAB
	};
	String[] lpchRenameCommonNameUser = {
			AdminpMoveUserInHier,
			AdminpRenameCommonNameUserInNAB
	};
	String[] lpchCommonXForRename = {
			AdminpRenameInTheACL,
			AdminpRenameInPersonDocs,
			AdminpRenamePersonInUnreadList,
			AdminpRenameInReadersAuthors,	/* Terminator check on admin server */
			ListSeperator,
			AdminpRenamePersonInFreeTime,
			AdminpRenamePersonInMailFile
	};
	...
	String[] lpchRenameGroup = {
			AdminpRenameGroupInNAB,
			AdminpRenameGroupInPersonDocs
	};
	String[] lpchCommonXForRenameGroup = {
			AdminpRenameGroupInTheACL,
			AdminpRenameGroupInReadersAuthors	/* Terminator check on admin server */
	};
	...
	*/
	String[] lpchUpgradeToRoamn = {
			AdminpCreateRoamingReplStubs,
			AdminpUpdateRoamingFields,
			AdminpMonitorRoamingReplStubs,
			AdminpUpdateRoamingState
	};
	...
	int DIRENTRYIDTRIM_LOCAL = (DIRENTRYID_PART_NOTES_SERVERNAME|DIRENTRYID_PART_NOTES_FILENAME);
	...
	/**  If {@link #NAME_GET_AB_TITLES} is specified then return the database path in place
	 * of the title for any database that has no title. {@link #NAME_DEFAULT_TITLES} has no
	...
	int MAXDIRENTRYID = (512 + 9 + MAXUSERNAME);
	...
	int PROCESS_GROUP_QUERY = MAXDWORD;
