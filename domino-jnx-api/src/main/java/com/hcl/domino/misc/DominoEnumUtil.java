...
public enum DominoEnumUtil {
	...
	/**
	 * Given a Domino number-style enum, returns the enum constant for the provided <code>Number</code>
	 * 
	 * @param <T> the number-backed enum type
	 * 
	 * @param clazz the number-backed enum class
	 * @param value the value to convert
	 * @return an {@link Optional} describing the corresponding enum value, or an empty one if none match
	 */
	public static <T extends INumberEnum<?>> Optional<T> valueOf(Class<T> clazz, Number value) {
	 ...
	 * @return an {@link Optional} describing the corresponding enum value, or an empty one if none match
	 */
	public static <N extends Number, T extends Enum<T> & INumberEnum<N>> Optional<T> valueOf(Class<T> clazz, short value) {
		for(T enumVal : clazz.getEnumConstants()) {
			long enumValue = enumVal.getLongValue();
			if(enumValue == value) {
				return Optional.of(enumVal);
	 ...
	 * @return an {@link Optional} describing the corresponding enum value, or an empty one if none match
	 */
	public static <N extends Number, T extends Enum<T> & INumberEnum<N>> Optional<T> valueOf(Class<T> clazz, int value) {
		for(T enumVal : clazz.getEnumConstants()) {
			long enumValue = enumVal.getLongValue();
			if(enumValue == value) {
				return Optional.of(enumVal);
	 ...
	 * @return an {@link Optional} describing the corresponding enum value, or an empty one if none match
	 */
	public static <N extends Number, T extends Enum<T> & INumberEnum<N>> Optional<T> valueOf(Class<T> clazz, long value) {
		for(T enumVal : clazz.getEnumConstants()) {
			long enumValue = enumVal.getLongValue();
			if(enumValue == value) {
				return Optional.of(enumVal);
	...
	/**
	 * Converts the provided collection of number-backed enum values to a bitfield numerical value.
	 * 
