...
/**
 * <p>
 * Implementation note: due to the underlying MQ capabilities, this
 * implementation of
 * {@link BlockingQueue} is not expected to implement the following methods:
 * </p>
 * <ul>
...
public interface MessageQueue extends BlockingQueue<String>, Closeable {
  ...
  public interface IMQCallback {
    public enum Action {
       ...
       * If the Buffer is smaller than the message, MQScan can return
       * ERR_MQ_BFR_TOO_SMALL.
       */
      Dequeue,
      /**
       * Remove the current message from the message queue and continue the
       * enumeration
       */
      Delete
    }
   ...
   * {@link MessageQueues#createAndOpen(String, int)} or
  ...
  /**
   * This function tests whether the specified message queue is in a QUIT state,
   * and returns TRUE if so. Otherwise, it returns FALSE.
   *
   ...
   * The message will be placed in the queue according to the value of its
   * priority argument -
   * higher priority messages will be enqueued ahead of lower priority
   * messages.<br>
   * <br>
   ...
   * If the message queue is empty, or if all messages in the queue have been
   * enumerated without
   ...
   * by returning the appropriate error codes to MQScan, the action routine can
   * specify that
   * messages are to be removed from the queue or skipped, or that enumeration is
   * to be
   * terminated immediately. See the reference entry for {@link IMQCallback} for
   * more details.<br>
   * <br>
