...
public interface DbDesign {
    ...
    @Deprecated
    public enum FormOrSubform {FORM, SUBFORM, ANY};
     ...
     * 
     * @param name the element name to restrict to
     * @return an {@link Optional} describing the {@link View}, or an empty one if no such view
     *      exists
     * @since 1.0.27
     */
     ...
     * 
     * @param name the element name to restrict to
     * @return an {@link Optional} describing the {@link Folder}, or an empty one if no such view
     *      exists
     * @since 1.0.27
     */
     ...
     * 
     * <p>Note: this method uses the value of the "$TITLE" field of the design element,
     * not the "$FileNames" of the file resource. These may diverge, such as when a
     * file resource has an alias assigned to it.</p>
     * 
    ...
    /**
     * Retrieves the named script library. This library will be of a specific subtype based on
     * its language.
     * 
    ...
    /**
     * Retrieves all script library design elements in the database. These libraries will be of
     ...
     */
    @Deprecated
    default GenericFormOrSubform<?> getFormOrSubform(FormOrSubform formSubformType, String name) {
    	switch(formSubformType) {
    	case FORM:
    		return getForm(name).orElse(null);
    	case SUBFORM:
    		return getSubform(name).orElse(null);
    	case ANY:
    	default:
    		Form form = getForm(name).orElse(null);
    		if(form != null) { return form; }
    		Subform subform = getSubform(name).orElse(null);
    		if(subform != null) { return subform; }
    		return null;
    	}
    }
    ...
    /**
     * Retrieves the database properties. This is, for implementation reasons, also known
     * as the icon note.
     * 
