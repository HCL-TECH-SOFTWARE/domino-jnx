...
public interface Replication {
	...
	/**
	 * Optional flags that can be used with {@link Replication#getReplicationHistory(Database, Set)}.
	 */
	public enum ReplicationHistoryFlags implements INumberEnum<Integer> {
		...
		ReplicationHistoryFlags(Integer value) {
			m_value = value;
		}
	...
	/**
	 * This function will return a stream of documents to the caller through several callback functions.<br>
	 * <br>
	 * It can be used to quickly and incrementally read a large number of docs from a database,
	 * skipping the transfer of item values where the item's sequence number is lower or equal a specified value
	 * (see <code>sinceSeqNum</code> parameter). In that case, items are returns with type {@link ItemDataType#TYPE_UNAVAILABLE}
	 * 
	 * @param db database
	 * @param noteIds note ID(s) of doc(s) to be retrieved
	 * @param docOpenFlags flags that control the manner in which the document is opened. This, in turn, controls what information about the doc is available to you and how it is structured. The flags are defined in {@link OpenDocumentMode} and may be or'ed together to combine functionality.
	 * @param sinceSeqNum since sequence number; controls which fields are accessible in the returned documents; e.g. if you specify a very high value, items with lower or equal sequence number have the type {@link ItemDataType#TYPE_UNAVAILABLE}
	 * @param controlFlags Flags that control the actions of the function during doc retrieval. The flags are defined in {@link GetDocumentsMode}.
	 * @param objectDb If binary objects are being retrieved ({@link GetDocumentsMode#SEND_OBJECTS} is used) and this value is not NULL, objects will be stored in this database and attached to the incoming docs prior to {@link IDocumentOpenCallback} being called.  
	 * @param getDocumentsCallback Called once before any others but only if going to a server that is R6 or greater. If {@link GetDocumentsMode#ORDER_BY_SIZE} is specified in options totalSize provides the approximate total size of the bytes to be returned in the documents and objects. These values are intended to be used for progress indication
	 * @param docOpenCallback This function is called for each document retrieved. If non-NULL, this is called for each document after all objects have been retrieved (if {@link GetDocumentsMode#SEND_OBJECTS} is specified)
	 * @param objectAllocCallback If {@link GetDocumentsMode#SEND_OBJECTS} is specified and <code>objectDb</code> is not NULL, this function is called exactly once for each object to provide the caller with information about the object's size and ObjectID. The intent is to allow for the physical allocation for the object if need be. It is called before the {@link IDocumentOpenCallback} for the corresponding document
	 * @param objectWriteCallback This function is called for each "chunk" of each object if {@link GetDocumentsMode#SEND_OBJECTS} is specified and <code>objectDb</code> is not NULL. For each object this will be called one or more times
	 * @param folderSinceTime {@link TemporalAccessor} containing a time/date value specifying the earliest time to retrieve documents from the folder. If {@link GetDocumentsMode#GET_FOLDER_ADDS} is specified this is the time folder operations should be retrieved from
	 * @param folderAddCallback If {@link GetDocumentsMode#GET_FOLDER_ADDS} is specified but {@link GetDocumentsMode#APPLY_FOLDER_ADDS} is not, this function is called for each document after the {@link IDocumentOpenCallback} function is called
	 */
	 ...
	 * @param force true to force document save even if the document was modified in the meantime
	 ...
	 * If {@link GetDocumentsMode#ORDER_BY_SIZE} is specified in options the two DWORD parameters,
	 * TotalSizeLow and TotalSizeHigh, provide the approximate total size of the bytes to be returned in the documents and objects. These values are intended to be used for progress indication
	 */
	@FunctionalInterface
	public interface IGetDocumentsCallback {
		 ...
		 * 
		 * @param totalSize The approximate total number of bytes that are going to be returned in the document stream
	...
	/**
	 * This function is called for each document retrieved. If non-NULL, this is called for each document
	 * after all objects have been retrieved (if {@link GetDocumentsMode#SEND_OBJECTS} is specified)
	 */
	@FunctionalInterface
	public interface IDocumentOpenCallback {
		 ...
		 * @param status if not empty, contains an error that occurred opening the document
		 * @return return {@link Action#STOP} to stop reading document data
		 */
	...
	/**
	 * If {@link GetDocumentsMode#SEND_OBJECTS} is specified and <code>objectDb</code> is not NULL,
	 ...
	 * for the object if need be. It is called before the {@link IDocumentOpenCallback} for the corresponding document
	 */
	@FunctionalInterface
	...
	/**
	 * This function is called for each "chunk" of each object if {@link GetDocumentsMode#SEND_OBJECTS}
	...
	public interface IObjectWriteCallback {
		 ...
		 * @param oldRRV Old  Record Relocation Vector
		 * @param status if not empty, contains any error that occurred
		 * @param buffer buffer containing a "chunk" of the object (as this function will be called one or more time for each object)
	...
	/**
	 * {@link GetDocumentsMode#GET_FOLDER_ADDS} is specified but {@link GetDocumentsMode#APPLY_FOLDER_ADDS} is not,
	 * this function is called for each document after the {@link IDocumentOpenCallback} function is called
	 */
	@FunctionalInterface
	public interface IFolderAddCallback {
		
		/**
		 * Called for each document after the {@link IDocumentOpenCallback} function is called
		 * 
		 * @param unid UNID
		 * @return return {@link Action#STOP} to stop reading document data
		 */
