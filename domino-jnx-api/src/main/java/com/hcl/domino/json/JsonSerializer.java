 ...
 *  
 * <p>Implementations of this service are expected to be provided by additional
 * integration modules, such as {@code domino-jnx-jsonp}.</p>
 * 
...
public interface JsonSerializer {
	 ...
	 * @throws IllegalStateException if there is no registered serializer implementation
	 */
	static JsonSerializer createSerializer() {
		return JNXServiceFinder.findServices(JsonSerializerFactory.class)
			.findFirst()
			.map(JsonSerializerFactory::newSerializer)
			.orElseThrow(() -> new IllegalStateException(MessageFormat.format("Unable to find a {0} service implementation", JsonDeserializerFactory.class.getName())));
	}
	...
	/**
	 * Sets whether to include a metadata object in the output JSON, using the {@value #PROP_METADATA} property.
	 ...
	 * 
	 * <p>By default, the only applied option is {@link HtmlConvertOption#XMLCompatibleHTML XMLCompatibleHTML=1}.
	 * Calling this method will remove that default in favor of user-specified options.</p>
	 * 
	...
	/**
	 * Specifies a custom processor for a named field. Calling this method will delegate handling of
	 * the named item on the document to the processor object.
	 * 
	 * <p>This processor is provided the contextual document and item name, and should return
	 * a JSON-compatible object. That is, one of:</p>
	 * 
	 * <ul>
	 *   <li>A primitive value</li>
	 *   <li>{@code String}</li>
	 *   <li>A {@link java.util.List List} of above or below objects or other such {@code List}s</li>
	 *   <li>A {@link java.util.Map Map} of {@code String} keys to above objects or other such {@code Map}s</li> 
	 * </ul>
	 * 
	 * @param propName the name of the property to process
	 * @param processor a {@link BiFunction} to process the context {@link Document} and item and produce
	 *       a compatible result
	 * @return this serializer
	 * @since 1.0.28
	 * @throws IllegalArgumentException if {@code propName} is empty
	 * @throws NullPointerException if {@code processor} or {@code propName} is {@code null}
	 */
	 ...
	 */
	default String toJsonString(Document doc) {
		return String.valueOf(toJson(doc));
	}
}
