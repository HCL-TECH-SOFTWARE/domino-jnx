...
public class NotesRequestFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		try {
			DominoClient client = DominoClientBuilder.newDominoClient().asUser(getUserName(request)).build();
			try {
				request.setAttribute(ATTRIBUTE_CLIENT, client);
	...
	private String getUserName(ServletRequest request) {
		...
		if (principal == null || principal.getName().equalsIgnoreCase("anonymous")) { //$NON-NLS-1$
			...
			return ldapNameToDomino(principal.getName());
		}
	}

	private static String ldapNameToDomino(String value) {
